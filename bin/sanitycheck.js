...
apps.forEach((app, appIdx) => {
  ...
  if (app.name.length > 20 && !app.shortName && isApp)
    ERROR(`App ${app.id} has a long name, but no shortName`);
  if (!Array.isArray(app.supports))
    ERROR(`App ${app.id} has no 'supports' field or it's not an array`);
  else {
  ...
  if (!app.version) WARN(`App ${app.id} has no version`);
    ...
    if (!fs.existsSync(appDir + "ChangeLog")) {
      if (app.version != "0.01") WARN(`App ${app.id} has no ChangeLog`);
    } else {
      var changeLog = fs.readFileSync(appDir + "ChangeLog").toString();
      var versions = changeLog.match(/\d+\.\d+:/g);
      if (!versions)
        ERROR(`No versions found in ${app.id} ChangeLog (${appDir}ChangeLog)`);
      var lastChangeLog = versions.pop().slice(0, -1);
      if (lastChangeLog != app.version)
        WARN(
          `App ${app.id} app version (${app.version}) and ChangeLog (${lastChangeLog}) don't agree`
        );
    }
  }
  if (!app.description) ERROR(`App ${app.id} has no description`);
  if (!app.icon) ERROR(`App ${app.id} has no icon`);
  if (!fs.existsSync(appDir + app.icon))
    ERROR(`App ${app.id} icon doesn't exist`);
  if (app.screenshots) {
    if (!Array.isArray(app.screenshots))
      ERROR(`App ${app.id} screenshots is not an array`);
  ...
  if (app.readme && !fs.existsSync(appDir + app.readme))
    ERROR(`App ${app.id} README file doesn't exist`);
  if (app.custom && !fs.existsSync(appDir + app.custom))
    ERROR(`App ${app.id} custom HTML doesn't exist`);
  if (app.customConnect && !app.custom)
    ERROR(`App ${app.id} has customConnect but no customn HTML`);
  if (app.interface && !fs.existsSync(appDir + app.interface))
    ERROR(`App ${app.id} interface HTML doesn't exist`);
  if (app.dependencies) {
    if (
      "object" == typeof app.dependencies &&
      ...
      Object.keys(app.dependencies).forEach((dependency) => {
        if (!["type", "app"].includes(app.dependencies[dependency]))
          ERROR(
            `App ${app.id} 'dependencies' must all be tagged 'type' or 'app' right now`
          );
      });
    } else ERROR(`App ${app.id} 'dependencies' must be an object`);
  }
  var fileNames = [];
  app.storage.forEach((file) => {
    ...
    if (char)
      ERROR(
        `App ${app.id} storage file ${file.name} contains invalid character "${char[0]}"`
      );
    if (fileNames.includes(file.name) && !file.supports)
      // assume that there aren't duplicates if 'supports' is set
      ERROR(`App ${app.id} file ${file.name} is a duplicate`);
    fileNames.push(file.name);
    allFiles.push({ app: app.id, file: file.name });
    if (file.url)
      if (!fs.existsSync(appDir + file.url))
        ERROR(`App ${app.id} file ${file.url} doesn't exist`);
    if (!file.url && !file.content && !app.custom)
    ...
    if (file.supports && !Array.isArray(file.supports))
      ERROR(`App ${app.id} file ${file.name} supports field is not an array`);
    if (file.evaluate) {
      try {
        ...
        console.log("=====================================================");
        ERROR(
          `App ${app.id}'s ${file.name} has evaluate:true but is not valid JS expression`
        );
      }
    }
    ...
    for (const key in file) {
      if (!STORAGE_KEYS.includes(key))
        ERROR(`App ${app.id} file ${file.name} has unknown key ${key}`);
    }
    ...
    if (file.name == app.id + ".img") {
      ...
      if (match) icon = Buffer.from(match[1], "base64");
        ...
        match = fileContents.match(
          /require\(\"heatshrink\"\)\.decompress\(\s*atob\(\s*\"([^"]*)\"\s*\)\s*\)/
        );
        if (match)
          icon = heatshrink.decompress(Buffer.from(match[1], "base64"));
        else
          ERROR(
            `JS icon ${file.name} does not match the pattern 'require("heatshrink").decompress(atob("..."))'`
          );
      }
      if (match) {
        if (icon[0] > 48 || icon[0] < 24 || icon[1] > 48 || icon[1] < 24) {
          if (GRANDFATHERED_ICONS.includes(app.id))
            WARN(
              `JS icon ${file.name} should be 48x48px (or slightly under) but is instead ${icon[0]}x${icon[1]}px`
            );
          else
            ERROR(
              `JS icon ${file.name} should be 48x48px (or slightly under) but is instead ${icon[0]}x${icon[1]}px`
  ...
  (app.data || []).forEach((data) => {
    if (!data.name && !data.wildcard)
      ERROR(`App ${app.id} has a data file with no name`);
    ...
    if (data.wildcard) {
      if (!isGlob(data.wildcard))
        ERROR(
          `App ${app.id} data file wildcard ${data.wildcard} does not actually contains wildcard`
        );
      if (data.wildcard.replace(/\?|\*/g, "") === "")
        ERROR(
          `App ${app.id} data file wildcard ${data.wildcard} does not contain regular characters`
        );
        ...
        WARN(
          `App ${app.id} data file wildcard ${data.wildcard} does not include app ID`
        );
    }
    let char = (data.name || data.wildcard).match(FORBIDDEN_FILE_NAME_CHARS);
    if (char)
      ERROR(
        `App ${app.id} data file ${
          data.name || data.wildcard
        } contains invalid character "${char[0]}"`
      );
    if ("storageFile" in data && typeof data.storageFile !== "boolean")
      ERROR(
        `App ${app.id} data file ${
          data.name || data.wildcard
        } has non-boolean value for "storageFile"`
      );
  ...
  if (fileNames.includes(app.id + ".settings.json"))
    WARN(
      `App ${app.id} uses storage file ${
        app.id + ".settings.json"
      } instead of data file`
    );
  if (fileNames.includes(app.id + ".json"))
    WARN(
      `App ${app.id} uses storage file ${app.id + ".json"} instead of data file`
    );
  ...
  dataNames.forEach((dataName) => {
    ...
    fileNames.forEach((fileName) => {
      if (glob.test(fileName)) {
        if (isGlob(dataName))
          WARN(
            `App ${app.id} storage file ${fileName} matches data wildcard ${dataName}`
          );
        else
          WARN(`App ${app.id} storage file ${fileName} is also listed in data`);
      }
    });
  });
  //console.log(fileNames);
  if (isApp && !fileNames.includes(app.id + ".app.js"))
    ERROR(`App ${app.id} has no entrypoint`);
  if (isApp && !fileNames.includes(app.id + ".img"))
    ERROR(`App ${app.id} has no JS icon`);
  if (app.type == "widget" && !fileNames.includes(app.id + ".wid.js"))
    ERROR(`Widget ${app.id} has no entrypoint`);
...
while ((fileA = allFiles.pop())) {
  if (VALID_DUPLICATES.includes(fileA.file)) return;
  ...
  allFiles.forEach((fileB) => {
    ...
    if (globA.test(nameB) || globB.test(nameA)) {
      if (isGlob(nameA) || isGlob(nameB))
        ERROR(
          `App ${fileB.app} ${typeB} file ${nameB} matches app ${fileA.app} ${typeB} file ${nameA}`
        );
      else
        WARN(
          `App ${fileB.app} ${typeB} file ${nameB} is also listed as ${typeA} file for app ${fileA.app}`
        );
    }
  });
}
