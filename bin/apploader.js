...
else if (args.length == 5 && args[2] == "install")
  cmdInstallApp(args[3], args[4]);
else {
...
function bangleSend(command, deviceAddress) {
  ...
  var RESET = true;
  var DEVICEADDRESS = "";
  if (deviceAddress !== undefined) DEVICEADDRESS = deviceAddress;
  ...
  return new Promise((resolve, reject) => {
    function foundDevice(dev) {
      ...
      connect(dev, function () {
        ...
        } else setTimeout(writeCode, 1000);
      });
    }
    ...
    function connect(dev, callback) {
      ...
      btDevice.connect(function (error) {
        ...
        log("BT> Connected");
        btDevice.discoverAllServicesAndCharacteristics(function (
          error,
          services,
          characteristics
        ) {
          ...
          txCharacteristic = findByUUID(
            characteristics,
            "6e400002b5a3f393e0a9e50e24dcca9e"
          );
          rxCharacteristic = findByUUID(
            characteristics,
    ...
    function write(data, callback) {
      ...
      function writeAgain() {
        if (flowControlPaused) {
          flowControlPaused--;
          if (flowControlPaused == 0) log("TIMEOUT - sending");
    ...
    noble.on("discover", function (dev) {
      ...
      if (a == DEVICEADDRESS) return foundDevice(dev);
      else if (
        DEVICEADDRESS == "" &&
        dev.advertisement.localName.indexOf("Bangle.js") == 0
      ) {
