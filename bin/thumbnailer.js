...
if (process.argv.length == 3) singleAppId = process.argv[2];
...
eval(
  require("fs")
    .readFileSync(
      __dirname + "/../../EspruinoWebIDE/emu/emulator_" + EMULATOR + ".js"
    )
    .toString()
);
eval(
  require("fs")
    .readFileSync(
      __dirname + "/../../EspruinoWebIDE/emu/emu_" + EMULATOR + ".js"
    )
    .toString()
);
eval(
  require("fs")
    .readFileSync(__dirname + "/../../EspruinoWebIDE/emu/common.js")
    .toString()
);
...
eval(
  require("fs")
    .readFileSync(__dirname + "/../core/lib/espruinotools.js")
    .toString()
);
eval(
  require("fs")
    .readFileSync(__dirname + "/../core/js/utils.js")
    .toString()
);
eval(
  require("fs")
    .readFileSync(__dirname + "/../core/js/appinfo.js")
    .toString()
);
...
function getThumbnail(appId, imageFn) {
  ...
  if (app.custom)
    ERROR(`App ${JSON.stringify(appId)} requires HTML customisation`);
...
setTimeout(function () {
  ...
  if (singleAppId) {
    getThumbnail(
      singleAppId,
      "screenshots/" + singleAppId + "-" + EMULATOR + ".png"
    );
    return;
  }

  var appList = apps.filter(
    (app) => (!app.type || app.type == "clock") && !app.custom
  );
  appList = appList.filter(
    (app) => !app.screenshots && app.supports.includes(DEVICEID)
  );
