...
if (DEVICE == "BANGLEJS") {
  var OUTFILE = path.join(
    ROOTDIR,
    "../Espruino/libs/banglejs/banglejs1_storage_default.c"
  );
  var APPS = [
    // IDs of apps to install
    "boot",
    "launch",
    "mclock",
    "setting",
  ...
  var OUTFILE = path.join(
    ROOTDIR,
    "../Espruino/libs/banglejs/banglejs2_storage_default.c"
  );
  var APPS = [
    // IDs of apps to install
    "boot",
...
function fileGetter(url) {
  ...
  var data;
  if (url.endsWith(".js") || url.endsWith(".json")) data = blob.toString();
  // allow JS/etc to be written in UTF-8
  else data = blob.toString("binary");
...
Promise.all(
  ...
  appfiles.forEach((file) => {
    ...
    header.fill(0);
    header.set(
      [fileLength, fileLength >> 8, fileLength >> 16, fileLength >> 24],
      0
    ); // length
