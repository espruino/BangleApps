...
function onFoundDeviceInfo(deviceId, deviceVersion) {
  ...
  if (deviceId != "BANGLEJS" && deviceId != "BANGLEJS2") {
    ...
    showToast(
      `You're using an old Bangle.js firmware (${deviceVersion}). You can <a href="${fwURL}" target="_blank">update with the instructions here</a>`,
      "warning",
      20000
    );
  }
...
function filterAppsForDevice(deviceId) {
  ...
  document.querySelector(".devicetype-nav span").innerText = device
    ? device.name
    : "All apps";
  ...
  if (!device) {
    if (deviceId !== undefined)
      showToast(
        `Device ID ${deviceId} not recognised. Some apps may not work`,
        "warning"
      );
    ...
    appJSON = originalAppJSON.filter((app) => {
      ...
      if (!app.supports) {
        console.log(
          `App ${app.id} doesn't include a 'supports' field - ignoring`
        );
        return false;
      }
...
window.addEventListener("load", (event) => {
  ...
  htmlToArray(document.querySelectorAll(".devicetype-nav .menu-item")).forEach(
    (button) => {
      button.addEventListener("click", (event) => {
        var a = event.target;
...
function onAppJSONLoaded() {
  let deviceId = getSavedDeviceId();
  if (deviceId !== undefined) filterAppsForDevice(deviceId);
