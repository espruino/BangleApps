(() => {
  ...
  function storeData() {
    ...
    filename = filename =
      "activepedom" + now.getFullYear() + month + now.getDate() + ".data"; //new file for each day
    dataFile = s.open(filename, "a");
    if (dataFile) {
      //check if filen already exists
  ...
  function calcSteps() {
    ...
    if (steps >= setting("stepThreshold")) {
      ...
      timerResetActive = setInterval(
        resetActive,
        setting("intervalResetActive")
      ); //reset active after timer runs out
      steps = 0;
    }
