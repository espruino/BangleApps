...
var boot = "";
if (require("Storage").hash) {
  // new in 2v11 - helps ensure files haven't changed
  var CRC =
    E.CRC32(require("Storage").read("setting.json")) +
    require("Storage").hash(/\.boot\.js/);
  boot += `if (E.CRC32(require('Storage').read('setting.json'))+require('Storage').hash(/\\.boot\\.js/)!=${CRC})`;
} else {
  var CRC =
    E.CRC32(require("Storage").read("setting.json")) +
    E.CRC32(require("Storage").list(/\.boot\.js/));
...
if (s.ble !== false) {
  if (s.HID) {
    ...
    if (s.HID == "joy")
      boot += `Bangle.HID = E.toUint8Array(atob("BQEJBKEBCQGhAAUJGQEpBRUAJQGVBXUBgQKVA3UBgQMFAQkwCTEVgSV/dQiVAoECwMA="));`;
    else if (s.HID == "kb")
      boot += `Bangle.HID = E.toUint8Array(atob("BQEJBqEBBQcZ4CnnFQAlAXUBlQiBApUBdQiBAZUFdQEFCBkBKQWRApUBdQORAZUGdQgVACVzBQcZAClzgQAJBRUAJv8AdQiVArECwA=="));`;
    /*kbmedia*/ else
      boot += `Bangle.HID = E.toUint8Array(atob("BQEJBqEBhQIFBxngKecVACUBdQGVCIEClQF1CIEBlQV1AQUIGQEpBZEClQF1A5EBlQZ1CBUAJXMFBxkAKXOBAAkFFQAm/wB1CJUCsQLABQwJAaEBhQEVACUBdQGVAQm1gQIJtoECCbeBAgm4gQIJzYECCeKBAgnpgQIJ6oECwA=="));`;
...
if (s.blerepl === false) {
  // If not programmable, force terminal off Bluetooth
  if (s.log == 2)
    boot += `_DBGLOG=require("Storage").open("log.txt","a");
...
if (!NRF.getSecurityStatus().connected) LoopbackA.setConsole();\n`;
    ...
    boot += `if (!NRF.getSecurityStatus().connected) Terminal.setConsole();\n`;
...
if (s.ble === false)
  boot += `if (!NRF.getSecurityStatus().connected) NRF.sleep();\n`;
// Set time
...
if (!Bangle.F_BEEPSET) {
  ...
  else if (s.beep == "vib" && !BANGLEJS2)
    boot += `Bangle.beep = function (time, freq) {
...
if (global.save)
  boot += `global.save = function() { throw new Error("You can't use save() on Bangle.js without overwriting the bootloader!"); }\n`;
// Apply any settings-specific stuff
if (s.options) boot += `Bangle.setOptions(${E.toJS(s.options)});\n`;
if (s.brightness && s.brightness != 1)
  boot += `Bangle.setLCDBrightness(${s.brightness});\n`;
if (s.passkey !== undefined && s.passkey.length == 6)
  boot += `NRF.setSecurity({passkey:${E.toJS(
    s.passkey.toString()
  )}, mitm:1, display:1});\n`;
if (s.whitelist)
  boot += `NRF.on('connect', function(addr) { if (!(require('Storage').readJSON('setting.json',1)||{}).whitelist.includes(addr)) NRF.disconnect(); });\n`;
...
if (!Bangle.setUI) {
  // assume this is just for F18 - Q3 should already have it
...
if (!E.showScroller) {
  // added in 2v11 - this is a limited functionality polyfill
...
if (!Bangle.appRect) {
  // added in 2v11 - polyfill for older firmwares
