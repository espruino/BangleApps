{
  ...
  let getSleepData = function () {
    ...
    if (getSettings().useTrueSleep)
      totalSleep = data.deepSleep + data.lightSleep;
    return {
      ...
      awakeSince: getMsPastMidnight(
        (global.sleeplog || {}).info ? global.sleeplog.info.awakeSince : 0
      )
    };
  };
  ...
  let getSleepScore = function (sleepData, settings, summaryData) {
    var components = {
      duration: {
        score: generateScore(
          sleepData.totalSleep / 60,
          settings.idealSleepHours
        ),
        weight: 0.6
      },
      deepSleep: {
        score: generateScore(sleepData.deepSleep / 60, settings.deepSleepHours),
        weight: 0.3
      }
    };
    if (summaryData.avgSleepTime > 0) {
      components.averageSleep = {
        score: generateScore(sleepData.totalSleep, summaryData.avgSleepTime),
        weight: 0.15
      };
    }
    if (summaryData.avgWakeUpTime > 0) {
      components.averageWakeup = {
        score: generateScore(sleepData.awakeSince, summaryData.avgWakeUpTime),
        weight: 0.1
      };
    }
  ...
  let getAllSleepScores = function () {
    ...
    return {
      durationScore: generateScore(
        sleepData.totalSleep / 60,
        settings.idealSleepHours
      ),
      deepSleepScore: generateScore(
        sleepData.deepSleep / 60,
        settings.deepSleepHours
      ),
      avgWakeUpScore:
        data.avgWakeUpTime > 0
          ? generateScore(sleepData.awakeSince, data.avgWakeUpTime)
          : 0,
      avgSleepTimeScore:
        data.avgSleepTime > 0
          ? generateScore(sleepData.totalSleep, data.avgSleepTime)
          : 0,
  ...
  let getCachedData = function () {
    ...
    data.sleepDuration = getSettings().useTrueSleep
      ? data.trueSleep
      : data.consecSleep;
    return data;
  };
  ...
  let getSummaryData = function (autoCalculate) {
    ...
    let cachedData = getCachedData();
    let bootData =
      require("Storage").readJSON("sleepsummary.bootdata.json", true) || {};
    let today = new Date().getDate();
    ...
    return Object.assign({}, avgData, cachedData, {
      scoreLastUpdated: bootData.promptDayShown || ""
    });
  };
  ...
  let recordSleepStats = function () {
    var sleepData = getSleepData();
    var data = getAvgData();
    data.avgWakeUpTime = averageNumbers(
      data.avgWakeUpTime,
      data.totalCycles,
      sleepData.awakeSince
    );
    data.avgSleepTime = averageNumbers(
      data.avgSleepTime,
      data.totalCycles,
