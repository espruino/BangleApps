{
  ...
  let getMsPastMidnight=function(unixTimestamp) {
    ...
    const seconds = dateObject.getSecon
{
  // Creator: RKBoss6
  // The calculations used are very resource-heavy, so we calculate once, and offload to a cache for the day.
  ...
  let getSleepScore=function(){
    ...
    return getWeightedScore({
      ...
      averageWakeup:
        {score:generateScore(getMsPastMidnight(global.sleeplog.info.awakeSince),summaryData.avgWakeUpTime),weight:0.1},
    });
  }
  ...
  let getAllSleepScores=function(){
    ...
    return {
      ...
      avgWakeUpScore: generateScore(getMsPastMidnight(global.sleeplog.info.awakeSince),data.avgWakeUpTime),
  ...
  let calcAndCache=function(){
    ...
    cachedData.wakeUpTime=getMsPastMidnight(global.sleeplog.info.awakeSince);
  ...
  function recordSleepStats(){
    ...
    var data=getAvgData();
    //Wakeup time
    var wakeUpTime=getMsPastMidnight(global.sleeplog.info.awakeSince);
  ...
  ds();
    ...
    const msPsastMidnight = (hours * 3600000) + (minutes * 60000) + (seconds * 1000) + milliseconds;
    return msPastMidnight;
  };
