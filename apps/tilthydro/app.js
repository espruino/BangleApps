...
function displayInfo(reading) {
  ...
  if (reading == "startup") {
    ...
    if (require("locale").temp(0).endsWith("F"))
      // check locale
...
function arrayBufferToHex(arrayBuffer) {
  return new Uint8Array(arrayBuffer)
    .slice()
    .map((x) => (256 + x).toString(16).substr(-2))
    .join("");
}
