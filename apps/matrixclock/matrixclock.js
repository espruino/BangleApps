...
class TextShard {
  ...
  isVisible() {
    return (
      this.y + (this.txt.length - this.length - 2) * SHARD_FONT_SIZE <
      g.getHeight()
    );
  }
  /**
...
function randomChar() {
  return String.fromCharCode(
    Math.floor(Math.random() * CHAR_CODE_LENGTH) + CHAR_CODE_START
  );
}
...
for (var i = 0; i < NO_SHARDS; i++) {
  shards.push(
    new TextShard(shard_x(i), 50 + Math.random() * 100, shard_length())
  );
}
...
function draw_clock() {
  ...
  if (
    last_draw_time == null ||
    now.getMinutes() != last_draw_time.getMinutes()
  ) {
...
function format00(num) {
  var value = num | 0;
  if (value > 99 || value < 0) throw "must be between in range 0-99";
