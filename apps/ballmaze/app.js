(() => {
  ...
  function playMaze(size) {
    ...
    let x,
      ...
      py,
      ppx,
      ppy, // previous positions (for erasing old image)
      vx,
      vy; // velocity
    ...
    function rcFromXy(x, y) {
      return [Math.floor((y / sH) * rows), Math.floor((x / sW) * cols)];
    }
    ...
    function generateMaze() {
      ...
      const // is room at row,col already linked?
    ...
    function drawBall() {
      ...
      const gx = -x / sW,
        gy = -y / sH;
      g.setColor(0.8, 0.8, 0.9)
        .fillCircle(x + (gx * r) / 5, y + (gy * r) / 5, r / 2)
        .setColor(0.85, 0.85, 0.95)
        .fillCircle(x + (gx * r) / 4, y + (gy * r) / 4.5, r / 2.5)
        .setColor(0.9, 0.9, 1)
        .fillCircle(x + (gx * r) / 3, y + (gy * r) / 3, r / 3.5)
        .setColor(1, 1, 1)
        .fillCircle(x + (gx * r) / 3, y + (gy * r) / 3, r / 6);
    }
