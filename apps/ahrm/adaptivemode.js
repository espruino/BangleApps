...
function write_data_pack(onHealthCopy) {
  ...
  let csvLine =
    timeStr +
    "," +
    Math.round(onHealthCopy.movement) +
    "," +
    onHealthCopy.accuracy +
    "," +
    onHealthCopy.bpm +
    "\n";
  let filename =
...
function pickdata() {
  ...
  var h = {
    movement: onMovement.total,
    accuracy: HRM.confidence,
    bpm: HRM.bpm
  };
  write_data_pack(h);
...
function HRMPOWER() {
  ...
  start_time = new Date();
  setTimeout(function () {
    if (Bangle.isHRMOn() && Math.abs(new Date() - start_time) >= 69 * 1000) {
      //turns off after one minute from power on
...
Bangle.on("HRM", function (h) {
  ...
  if (
    h.confidence >= 90 &&
    Math.abs(Bangle.getHealthStatus().bpm - h.bpm) < 1
  ) {
    Bangle.setHRMPower(0, "ahrm");
  }
});
...
function accelDataProcessing() {
  ...
  if (!changed) {
    if (mv_sd >= 50 && mv_sd < 80 && mv_avg < 130) {
      // every 5 minutes
      changed = true;
      mode_prec = mode;
      mode = 4;
