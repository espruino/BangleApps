(function () {
  ...
  function check() {
    if (Bangle.isCharging() || E.getBattery() > setting("percentage")) {
    ...
    require("notify").show({
      ...
      icon: require("heatshrink").decompress(
        atob("jEYwgfchnM5nABaQJCBoQLSAhAL/Bf6bHAAYLpACgA==")
      ),
      title: "Low Battery",
      render: (a) => {
        g.setFont("6x8", 2)
          .setFontAlign(-1, 0)
          .setColor(-1)
          .drawString("Battery: ", a.x + 8, a.y + a.h / 2)
          .setColor(0xf800)
          .drawString(`${E.getBattery()}%`, a.x + 8 + 100, a.y + a.h / 2);
      },
    });
    if (
      setting("buzz") &&
