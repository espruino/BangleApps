...
class CSCSensor {
  ...
  updateBatteryLevel(event) {
    if (event.target.uuid == "0x2a19")
      this.setBatteryLevel(event.target.value.getUint8(0));
  }

  drawBatteryIcon() {
    g.setColor(1, 1, 1)
      .drawRect(10, 55, 20, 75)
      .fillRect(14, 53, 16, 55)
      .setColor(0)
      .fillRect(11, 56, 19, 74);
  ...
  updateScreenRevs() {
    var dist =
      (this.distFactor *
        (this.lastRevs - this.lastRevsStart) *
        this.wheelCirc) /
      63360.0;
    ...
    var avespeed =
      this.movingTime > 3
        ? Math.round((10 * dist) / (this.movingTime / 3600)) / 10
        : 0;
    ...
    if (this.screenInit) {
      for (var i = 0; i < 6; ++i) {
        ...
        g.setColor(0.5, 0.5, 0.5)
          .drawRect(87, 48 + i * 32, 239, 48 + (i + 1) * 32)
          .drawLine(0, 239, 239, 239); //.drawRect(0, 48, 87, 239);
        g.moveTo(0, 80)
          .lineTo(30, 80)
          .lineTo(30, 48)
          .lineTo(87, 48)
    ...
    if (event.target.uuid == "0x2a5b") {
      if (event.target.value.getUint8(0, true) & 0x2) {
        ...
        if (crankTime > this.lastCrankTime) {
          this.cadence =
            ((crankRevs - this.lastCrankRevs) /
              (crankTime - this.lastCrankTime)) *
            (60 * 1024);
          qChanged = true;
        }
...
function connection_setup() {
  ...
  NRF.requestDevice({ filters: [{ services: ["1816"] }] })
    .then(function (d) {
      device = d;
