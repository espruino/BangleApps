...
const INDICATORS = INDICATOR_LAYOUT.reduce((a, c, i) => {
  ...
  (l, r) =>
    (l.type - r.type) * FIELD_COUNT_X * FIELD_COUNT_Y +
    (l.position[0] +
      l.position[1] * FIELD_COUNT_X -
...
let field_states = [
  ...
  new Array(FIELD_COUNT_X * FIELD_COUNT_Y).fill(0),
];
let current = [
  [0, 0],
  [0, 0],
...
function getHitCount() {
  return field_states[player].reduce((v, state) => (state == 3 ? v + 1 : v), 0);
}
...
function drawField(x, y, selected) {
  ...
  switch (field_state) {
    case 2:
      ...
      g.fillCircle(
        x1 + FIELD_WIDTH[phase] / 2 + 1,
        y1 + FIELD_HEIGHT[phase] / 2 + 1,
        FIELD_WIDTH[phase] / 2 - 3
      );
      break;
    case 3:
...
function determineChanges() {
  to_rem = to_add;
  to_add = null;
  if (selected[0] == current[player][0] && selected[1] == current[player][1])
    return;
...
function drawShipPlacements() {
  ...
  placements[player].forEach((p) =>
    drawShip.apply(null, p.position.concat([SHIP_COLOR_PLACED]))
  );
  if (to_add) {
