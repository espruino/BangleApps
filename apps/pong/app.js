...
const intersects = (circ, rect, right) => {
  ...
  if (c.y - r < rect.pos.y + rect.height && c.y + r > rect.pos.y) {
    if (right) {
      return (
        c.x + r > rect.pos.x - rect.width * 2 && c.x < rect.pos.x + rect.width
      );
    } else {
      return (
        c.x - r < rect.pos.x + rect.width * 2 && c.x > rect.pos.x - rect.width
...
Ball.prototype.reset = function () {
  ...
  var x =
    scores[0] < scores[1] || (scores[0] === 0 && scores[1] === 0)
      ? -this.speed
      : this.speed;
  var bounceAngle = Math.PI / 6;
  this.velocity = new Vector(
    x * Math.cos(bounceAngle),
    this.speed * -Math.sin(bounceAngle)
  );
...
Ball.prototype.restart = function () {
  this.reset();
  ai.pos = new Vector(
    SCREEN_WIDTH - ai.width * 2,
    SCREEN_WIDTH / 2 - ai.height / 2
  );
  player.pos = new Vector(
    player.width * 2,
    SCREEN_WIDTH / 2 - player.height / 2
  );
...
Ball.prototype.bounce = function (directionX, directionY, player) {
  if (player) return this.bouncePlayer(directionX, directionY, player);
...
function Player(right) {
  ...
  this.height = 30;
  this.pos = new Vector(
    right ? SCREEN_WIDTH - this.width : this.width,
    SCREEN_WIDTH / 2 - this.height / 2
  );
...
AI.prototype.update = function () {
  ...
  var randomizedY =
    ball.ballReturn < 3 ? y : y + (aiSpeedRandom * this.height) / 2;
...
`;
  ...
  var arrows = [
    Graphics.createImage(topArrowString),
