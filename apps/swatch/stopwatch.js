  ...
  function () {
    ...
    if (started)
      displayInterval = setInterval(function () {
        ...
        if (
          Math.floor((last - tStart) / 1000) !=
            Math.floor((tCurrent - tStart) / 1000) ||
          Math.floor((last - tTotal) / 1000) !=
            Math.floor((tCurrent - tTotal) / 1000)
        )
          drawsecs();
        else drawms();
      }, 20);
...
setWatch(
  function () {
    ...
    if (!started) {
      ...
      var filename =
        "swatch-" +
        new Date().toISOString().substr(0, 16).replace("T", "_") +
        ".json";
