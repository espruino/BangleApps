...
function getApps() {
  ...
  const raw_apps = Storage.list(/\.info$/)
    .filter((app) => app.endsWith(".info"))
    .map(
      (app) => Storage.readJSON(app, 1) || { name: "DEAD: " + app.substr(1) }
    )
...
function render() {
  ...
  const visibleApps = APPS.filter(
    (app) =>
      app.x >= STATE.offset - HALF && app.x <= STATE.offset + WIDTH - HALF
  );
  ...
  visibleApps.forEach((app) => {
    ...
    if (app.special) {
      const fontSize =
        process.env.HWVERSION == 2 ? 4 : settings.highres ? 6 : 2;
    ...
    if (icon) {
      ...
      try {
        ...
        if (process.env.HWVERSION == 1) {
          ...
          rescale = settings.highres
            ? scale * ORIGINAL_ICON_SIZE
            : scale * (ORIGINAL_ICON_SIZE / 2);
  ...
  if (settings.debug) {
    ...
    g.setFont(
      process.env.HWVERSION == 2 ? "6x8" : settings.highres ? "6x8" : "4x6",
      fontSize
    );
    // steal the bottom line, and print the duration
    g.clearRect(
      0,
      HEIGHT - (process.env.HWVERSION == 1 && !settings.highres ? 8 : 24),
      WIDTH,
      HEIGHT
    );
  ...
  if (STATE.offset >= STATE.target - THRESHOLD && STATE.offset < STATE.target)
