...
function drawStar(
  ...
  var dec = Math.asin(
    Math.sin(theta) * Math.cos(starDE) * Math.cos(starRA + zeta) +
      Math.cos(theta) * Math.sin(starDE)
  );
  var ascen =
    Math.atan2(
      Math.cos(starDE) * Math.sin(starRA + zeta),
      Math.cos(theta) * Math.cos(starDE) * Math.cos(starRA + zeta) -
        Math.sin(theta) * Math.sin(starDE)
    ) + z;
  ...
  if (alt >= 0) {
    ...
    var col = 1;
    if (starMag <= 1.5) col = 1;
    else if (starMag > 1.5 && starMag < 2) col = 0.9;
    else if (starMag >= 2 && starMag < 3) col = 0.7;
    else if (starMag >= 3 && starMag < 3.5) col = 0.5;
    else col = 0.3;

    g.setColor(col, col, col);
    g.fillCircle(x, y, magnitude);
    if (starMag < 1 && settings.starnames) g.drawString(starInfo[3], x, y + 2);
    g.flip();
  }
}
  ...
  if (settings.constellations) {
    ...
    while (lineend >= 0) {
      ...
      for (j = 0; j < lines.length; j++) {
        ...
        } else constelationShowing = false;
        g.flip();
      }
