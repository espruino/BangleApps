...
function alarm() {
  if (alarm_length > 0) {
    ...
    clearInterval(alarm);
    if (trigger_count > 1) Bangle.setHRMPower(0);
  }
}
...
function checkHR() {
  ...
  if (HR_samples.length == 5) {
    ...
    if (average_HR < target_heartrate && stdev_HR < deviation_threshold) {
      ...
      trigger_count++;
      var csv = [0 | getTime(), launchtime, average_HR, stdev_HR];
      file.write(csv.join(",") + "\n");
...
Bangle.on("HRM", function (hrm) {
  if (trigger_count < 2) {
    ...
    // ignore the first one as it's usually rubbish
    else {
      currentBPM = hrm.bpm;
      lastBPM = currentBPM;
    }
