...
function timeToText(t) {
  ...
  if (hrs === 0)
    ...
    text =
      "0" + hrs + ":" + ("0" + mins).substr(-2) + ":" + ("0" + secs).substr(-2);
...
function stopStart() {
  ...
  if (running) tStart = Date.now() + tStart - tCurrent;
...
var bigPlayPauseBtn = new BUTTON(
  "big",
  0,
  (3 * h) / 4,
  w,
...
var smallPlayPauseBtn = new BUTTON(
  "small",
  w / 2,
  (3 * h) / 4,
  w / 2,
...
Bangle.on("touch", function (button, xy) {
  ...
  if (!running && tCurrent == tTotal && bigPlayPauseBtn.check(xy.x, xy.y))
    return;

  // paused and hit play
  if (!running && tCurrent != tTotal && smallPlayPauseBtn.check(xy.x, xy.y))
    return;
