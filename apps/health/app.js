...
function stepsPerDay() {
  ...
  require("health").readDailySummaries(
    new Date(),
    (h) => (data[h.day] += h.steps)
  );
  g.clear(1);
  Bangle.drawWidgets();
  g.reset();
...
function get_data_length(arr) {
  ...
  for (var i = arr.length - 1; i > 0 && arr[i] == 0; i--) nlen--;
...
function drawBarChart() {
  ...
  for (bar = 1; bar < 10; bar++) {
    ...
    if (chart_index + bar - 1 >= 0 && chart_index + bar - 1 < data_len)
      bar_top =
        bar_bot - (100 * chart_data[chart_index + bar - 1]) / chart_max_datum;
    else bar_top = bar_bot;
...
function setButton(fn) {
  ...
  if (process.env.HWVERSION == 1) btn = setWatch(fn, BTN2);
  else btn = setWatch(fn, BTN1);
}
