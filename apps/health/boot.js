...
Bangle.on("health", (health) => {
  ...
  function getRecordIdx(d) {
    return (
      DB_RECORDS_PER_DAY * (d.getDate() - 1) +
  ...
  function getRecordData(health) {
    return String.fromCharCode(
      health.steps >> 8,
      health.steps & 255, // 16 bit steps
      health.bpm, // 8 bit bpm
      Math.min(health.movement / 8, 255)
    ); // movement
  }
  ...
  if (health.bpmCnt) health.bpm /= health.bpmCnt;
  if (health.movCnt) health.movement /= health.movCnt;
