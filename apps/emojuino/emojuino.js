...
const TX =
  "k8XwkBiIAYEYogLHBAUIiBNKGxooKEggvJCYYHDKxAMFAoRrOCRAsHCYqbNHQibLKAauOLBCJHQw6JMQBIJBRJDWJThK5JJJi5KbpaJKFBaKEE5ybGHRhcOACEQA";
...
const UNICODE_CODE_POINT_ELIDED_UUID = [
  0x49, 0x6f, 0x49, 0x44, 0x55, 0x54, 0x46, 0x2d, 0x33, 0x32,
];
...
function transmitEmoji(image, codePoint, duration) {
  ...
  require("ble_eddystone_uid").advertise(
    UNICODE_CODE_POINT_ELIDED_UUID,
    instance
  );
  isTransmitting = true;
...
function handleDrag(event) {
  ...
  if (isFingerReleased) {
    let isHorizontalDrag =
      Math.abs(lastDragX) >= Math.abs(lastDragY) && lastDragX !== 0;
