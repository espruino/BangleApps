...
function drawMixedClock(force) {
  if ((force || Bangle.isLCDOn()) && buf.buffer) {
    ...
    buf.drawString(
      isEn ? " " + dateArray[2] : locale.month(date, true),
      4,
      176,
      true
    );
    buf.setFontAlign(1, 0);
    buf.drawString(
      isEn ? locale.month(date, true) : " " + dateArray[2],
      237,
      16,
      true
    );
    ...
    buf.fillPoly(
      setLineWidth(Center.x, Center.y, point[0], point[1], Widths.minute)
    );
    // draw new hour hand
    point = rotatePoint(
      0,
      Radius.hour,
      ((hour % 12) * 30 + date.getMinutes() / 2) | 0
    );
    buf.fillPoly(
      setLineWidth(Center.x, Center.y, point[0], point[1], Widths.hour)
    );
