  ...
  var l = {
    ...
    draw: function (rowmin, rowmax) {
      ...
      if (rowmin === undefined && options.title) {
        ...
        g.setColor("#FFF");
        g.setFontAlign(0, 0);
        g.drawString(options.title.toUpperCase(), 88, 13);

        var bar_width =
          (176 - g.stringWidth(options.title.toUpperCase()) - 40) / 2;

        if (bar_width > 0)
          [0, 176 - bar_width].forEach((v) => {
            for (let i = 5; i < 20; i += 3) {
              g.drawLine(v, i, v + bar_width, i);
            }
          });
      }
