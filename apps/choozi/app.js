...
function animateChoice(target) {
  ...
  for (var i = 0; true; i++) {
    ...
    if (i < animStartSteps || (speed < maxSpeed && !allowDecel)) {
      ...
      if (
        !allowDecel &&
        angle < decelFromAngle &&
        angle + speed >= decelFromAngle
      )
        allowDecel = true;
      if (allowDecel) speed = speed - accel;
      if (speed < minSpeed) speed = minSpeed;
      if (speed == minSpeed && angle < target && angle + speed >= target)
        return;
    }
...
function drawN() {
  ...
  if (N < maxN)
    g.fillPoly([
      centreX - 6,
      centreY - fontSize / 2 - 7,
      centreX + 6,
      centreY - fontSize / 2 - 7,
      centreX,
      centreY - fontSize / 2 - 14,
    ]);
  if (N > minN)
    g.fillPoly([
      centreX - 6,
      centreY + fontSize / 2 + 5,
      centreX + 6,
      centreY + fontSize / 2 + 5,
      centreX,
      centreY + fontSize / 2 + 12,
    ]);
}
