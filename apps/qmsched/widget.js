(function () {
  WIDGETS["qmsched"] = {
    area: "tl",
    width:
      (require("Storage").readJSON("setting.json", 1) || {}).quiet | 0 ? 24 : 0,
    draw: function () {
      const mode =
        (require("Storage").readJSON("setting.json", 1) || {}).quiet | 0;
      if (mode === 0) {
        // Off
        if (this.width !== 0) {
          this.width = 0;
      ...
      g.setColor(process.env.HWVERSION === 2 ? 1 : 0.8, 0, 0).fillCircle(
        x,
        y,
        8
      );
      ...
      g.setColor(1, 1, 0)
        ...
        .fillRect(x - 1, y - 5, x + 1, y + 5)
        .drawLine(x, y - 6, x, y + 6) // top+bottom
        .drawLine(x + 5, y - 3, x + 3, y - 5)
        .drawLine(x - 5, y - 3, x - 3, y - 5); // wriggles
    },
  };
})();
