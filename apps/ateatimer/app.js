...
let timerDuration = (() => {
  let data = require("Storage").readJSON("ateatimer.json", true);
  return data && data.duration ? data.duration : 4 * 60;
})(); // Load default from storage or set to 4 minutes
...
function saveDefaultDuration() {
  require("Storage").writeJSON("ateatimer.json", { duration: timerDuration });
}
