...
var settings = require("Storage").readJSON("zone.json",1)||{};
var plotTrack;
...
function drawCircle(lat, lon, radius) {
  var center = m.latLonToXY(lat, lon);
  var scaleFactor = m.getScaleFactor();
  var pixelRadius = radius / scaleFactor;
  g.setColor(1, 0, 0); // Red color
  g.drawCircle(center.x, center.y, pixelRadius);
}
...
Bangle.setCompassPower(settings.dirSrc === 2, "zone");
...
if (HASWIDGETS) {
  ...
  function writeSettings() {
    ...
    require("Storage").writeJSON("zone.json",settings);
  }
