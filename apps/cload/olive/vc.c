...
#elif VC_PLATFORM == VC_ESPRUINO_PLATFORM
...
void put_pixel(char *fb, int x, int y, int c)
{
	setPixel(fb, x, y, c);
}

void big_pixel(char *fb, int x1, int y1, int c, int zoom)
{
	for (int y=0; y<zoom; y++) {
		for (int x=0; x<zoom; x++) {
			put_pixel(fb, x+x1, y+y1, to_3bit(c));
		}
	}
}

void olive_step(float dt, char *fb)
{
	Olivec_Canvas oc_src = vc_render(dt);
	int zoom = 3;
	if (oc_src.width * zoom > WIDTH) {
		msg("Bad");
		return;
	}
	//memset(fb, 0x8, WIDTH*HEIGHT/3);
	for (int y=0; y<oc_src.height; y++) {
		for (int x=0; x<oc_src.width; x++) {
			//put_pixel(x, y, ..);
			int p = y*oc_src.width + x;
			big_pixel(fb, x*zoom, y*zoom, oc_src.pixels[p], zoom);
		}
	}
	//msg("Good");
}
