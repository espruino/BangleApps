...

{
  ...
  let drawSideBar3=function() {
    ...
    g.drawImage(sunrise_img, 113, 8, { scale: 1 });
    setTextColor();
    setSmallFont();
    g.setFontAlign(0, -1);
    g.drawString(sunRise, 145, 72);

    g.setColor('#000'); // sunset black
    g.drawImage(sunset_img, 113, 88, { scale: 1 });
    setTextColor();
    setSmallFont();
    g.setFontAlign(0, -1);
    g.drawString(sunSet, 145, 152);
  }

  let drawDateAndCalendar=function(x, y, dy, dd, mm, yy) {
    // day
    setTextColor();
    setSmallFont();
    g.setFontAlign(0, -1);
