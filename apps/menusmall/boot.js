...
E.showMenu = function (items) {
  ...
  var l = {
    ...
    draw: function (rowmin, rowmax) {
      ...
      while (rows--) {
        ...
        if ("object" == typeof item) {
          ...
          if (l.selectEdit && idx == options.selected) {
            xo -= 24 + 1;
            g.setColor(g.theme.bgH).fillRect(
              xo - (g.stringWidth(v) + 4),
              iy,
              x2,
              iy + options.fontHeight - 1
            );
            g.setColor(g.theme.fgH).drawImage(
              "\x0c\x05\x81\x00 \x07\x00\xF9\xF0\x0E\x00@",
              xo,
              iy + (options.fontHeight - 10) / 2,
              { scale: 2 }
            );
          }
          g.setFontAlign(1, -1);
      ...
      g.setFontAlign(-1, -1);
      g.setColor(idx < menuItems.length ? g.theme.fg : g.theme.bg).fillPoly([
        72, 166, 104, 166, 88, 174,
      ]);
      g.flip();
    },
    select: function () {
    ...
    move: function (dir) {
      if (l.selectEdit) {
        ...
        options.selected =
          (dir + options.selected + menuItems.length) % menuItems.length;
