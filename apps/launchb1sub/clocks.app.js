...
var clocks = s.list(/\.info$/).map(app=>{var a=s.readJSON(app,1);return a&&{name:a.name,type:a.type,icon:a.icon,sortorder:a.sortorder,src:a.src};}).filter(app=>app && (app.type=="clock"));
...
function drawMenu() {
  ...
  g.setColor((clocks.length>n+menuScroll) ? g.theme.fg : g.theme.bg);
  g.fillPoly([m,h-7,m-14,h-21,m+14,h-21]);
  // draw
  g.setColor(g.theme.fg);
  for (var i=0;i<n;i++) {
    var app = clocks[i+menuScroll];
    if (!app) break;
    var y = 24+i*64;
...
Bangle.setUI("updown",dir=>{
  if (dir) {
    selected += dir;
    if (selected<0) selected = clocks.length-1;
    if (selected>=clocks.length) selected = 0;
    drawMenu();
  } else {
    if (!clocks[selected].src) return;
    if (require("Storage").read(clocks[selected].src)===undefined) {
      E.showMessage("App Source\nNot found");
      setTimeout(drawMenu, 2000);
    } else {
      E.showMessage("Loading...");
      load(clocks[selected].src);
    }
  }
});
Bangle.loadWidgets();
