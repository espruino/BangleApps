...
var onHRMRaw=function(hrm) {
    ...
    if (hasLock == false) {
        E.showMessage(
            "Measuring...\nRemain Still",
            {
                uploadProgress: raw_HR_array.length
            }
        );
    ...
    if (raw !== undefined && raw !== null) {
        ...
        if (pulsecount < raw_HR_array.length) {
            ...
            if (
                pulsecount == raw_HR_array.length
            ) {
                turn_off();
            } 
        }
...
function find_peaks() {
    ...
    var minPeakDistance = Math.floor(
        (sample_frequency * 2) * 60 / hr_max * 0.70
    );

    var maxPeakDistance = Math.floor(
        (sample_frequency * 2) * 60 / hr_min * 1.40
    );
...
function calculate_HRV() {
   ...
   //     Bangle.buzz(200, 1);
    ...
    var RMSSD = Math.sqrt(
        squaredDifferences / (temp_array.length - 1)
    );

    E.showMessage(
        "HRV:" + RMSSD.toFixed(0) +
        "\nHR:" + calculatedHR.toFixed(0) +
        "\nSample Count:" + temp_array.length,
        {
            title: "Complete"
        }
    );
