...
function checkForTime() {
  ...
  if (
    currentMinute >= d_startMins &&
    currentMinute <= d_endMins &&
    settings.enabled == true
  ) {
    load("hrvmeasure.app.js");
    return;
  } else if (currentMinute > d_endMins) {
    let shouldCalculateAverage =
      !dataSaved.hrvDailyAverages[0] ||
      (currentMinute > d_endMins &&
        !isTimestampToday(dataSaved.hrvDailyAverages[0].timestamp));
