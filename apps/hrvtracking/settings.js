(function (back) {
  var settings = require("Storage").readJSON(
    "hrvtracking.settings.json",
    1
  ) || {
      ...
      /*LANG*/ "Start Time": {
        ...
        onchange: (v) => {
          if (v >= settings.endTime) {
            E.showAlert(/*LANG*/ "Cannot be greater than end time.").then(
              () => {
                showMainMenu();
              }
            );
      ...
      /*LANG*/ "End Time": {
        ...
        onchange: (v) => {
          if (v <= settings.startTime) {
            E.showAlert(/*LANG*/ "Cannot be less than start time.").then(() => {
              showMainMenu();
            });
          } else {
            settings.endTime = v;
