...
class BTN_layer extends Layer {
  ...
  push_buttons(points, in_string, start_p) {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      )}${spacer}${in_string.substring(
        start_p + chunk_size / 2,
        start_p + chunk_size
      )}`;

      btn_label = uppercase === 1 ? substr.toUpperCase() : substr;
...
class TextBox {
  ...
  render() {
    ...
    var align_right = (0, 0);
    alignment =
      g.stringWidth(this.text) < g.getWidth() ? align_center : align_right;
...
var box = new TextBox(_screen_mid, 12, editable_buf, white);
