...
function PrintHelp() {
  ...
  g.setFontVector(v_font2size).drawString(
    "To test the UI, try:",
    5,
    y_wg_bottom - 10 * v_font3size
  );
  g.flip();
...
function ChangeColorBannerArea(v_color) {
  ...
  else if (v_color == "#51504f") v_color = 0x7800;
  // Maroon
  else if (v_color == 0x7800) v_color = "#CC3333";
  //coldarkred
...
function DrawRoundOption(x_obj1, y_obj1, x_obj2, y_obj2, i_area) {
  ...
  var img_obj_check = {
    ...
    palette: new Uint16Array([
      65535, 28436, 38872, 4838, 6951, 34711, 26355, 24242, 32630, 36792, 30517,
      32598, 40953, 26323, 9129, 7016,
    ]),
    buffer: E.toArrayBuffer(
      atob(
        ""
      )
    ),
  };
  var img_obj_uncheck = {
    ...
    transparent: 0,
    palette: new Uint16Array([
      65535, 63422, 9532, 13789, 59197, 57084, 34266, 28220, 63390, 65503,
      61310, 61277, 57116, 55003, 61309, 40604,
    ]),
    buffer: E.toArrayBuffer(
      atob(
...
function PrintUserInput(boton) {
  ...
  //all input but not swipe
    ...
    if (boton == "Touch 1" || boton == "Touch 2") {
      ...
      if (v_mode_debug == 1)
        console.log(
          "array[" + v_selected_row + "]=" + array_r_option[v_selected_row]
        );
    }
  }
...
function DrawBangleButtons() {
  ...
  g.drawString(
    "Quit",
    x_max_screen - g.stringWidth("Quit"),
    y_wg_bottom - v_font1size - 2
  );
  ...
  if (v_mode_debug == 1) {
    ...
    g.drawString(
      v_str_version,
      x_max_screen - g.stringWidth(v_str_version),
      y_wg_bottom - v_font1size * 3
    );
  }
