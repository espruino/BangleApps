...
GPS.prototype.getGPSTime = function () {
  ...
  if (
    this.last_fix !== undefined &&
    this.last_fix.time !== undefined &&
    this.last_fix.time.toUTCString !== undefined &&
...
GPS.prototype.getWPdistance = function () {
  ...
  if (
    this.wp_current.name === "E-WPT" ||
    this.wp_current.name === "NONE" ||
    this.wp_current.lat === undefined ||
    this.wp_current.lat === 0
  )
    return 0;
  else return this.calcDistance(this.last_fix, this.wp_current);
};
...
GPS.prototype.getWPbearing = function () {
  ...
  if (
    ...
    return 0;
  else return this.calcBearing(this.last_fix, this.wp_current);
};
...
STOPWATCH.prototype.timeToText = function (t) {
  ...
  if (hrs === 0) text = ("0" + mins).substr(-2) + ":" + ("0" + secs).substr(-2);
  else
    text =
      "" + hrs + ":" + ("0" + mins).substr(-2) + ":" + ("0" + secs).substr(-2);
...
STOPWATCH.prototype.stopStart = function () {
  ...
  if (this.running) this.tStart = Date.now() + this.tStart - this.tCurrent;
...
STOPWATCH.prototype.drawLaptimes = function () {
  ...
  for (let i in this.lapTimes) {
    g.drawString(
      this.lapTimes.length - i + ": " + this.timeToText(this.lapTimes[i]),
      4,
      this.timeY + 40 + i * 24
    );
    if (++laps > 5) break;
