(() => {
  ...
  WIDGETS["wpedom"] = {
    ...
    width: 26,
    redraw: function () {
      // work out the width, and queue a full redraw if needed
      let stps = stp_today.toString();
      let newWidth = 24;
      if (setting("hide")) newWidth = 0;
      else {
        if (setting("large")) {
          newWidth = 12 * stps.length + 3;
          if (setting("progress")) newWidth += 24;
        }
      }
    ...
    draw: function () {
      ...
      if (stp_today > 99999) stp_today = stp_today % 100000; // cap to five digits + comma = 6 characters
      let stps = stp_today.toString();
      ...
      if (setting("progress")) {
        ...
        // blue or dark green
        g.setColor(left ? "#08f" : "#080").fillCircle(
          this.x + half,
          this.y + half,
          half
        );
        if (left) {
          ...
          g.setColor(g.theme.bg).fillPoly(
            g.transformVertices(p, { x: this.x, y: this.y })
          );
        }
        g.reset();
      }
      if (setting("large")) {
        ...
        g.drawImage(
          atob("CgoCLguH9f2/7+v6/79f56CtAAAD9fw/n8Hx9A=="),
          this.x + (w - 10) / 2,
          this.y + 2
        );
      }
