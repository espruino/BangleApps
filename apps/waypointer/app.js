...
var arrow_img = require("heatshrink").decompress(
  atob(
    "lEowIPMjAEDngEDvwED/4DCgP/wAEBgf/4AEBg//8AEBh//+AEBj///AEBn///gEBv///wmCAAImCAAIoBFggE/AkaaEABo="
  )
);

function flip1(x, y) {
  g.drawImage(
    { width: 160, height: 160, bpp: 1, buffer: buf1.buffer, palette: pal_by },
    x,
    y
  );
  buf1.clear();
...
function flip2_bw(x, y) {
  g.drawImage(
    { width: 80, height: 40, bpp: 1, buffer: buf2.buffer, palette: pal_bw },
    x,
    y
  );
  buf2.clear();
...
function flip2_bb(x, y) {
  g.drawImage(
    { width: 80, height: 40, bpp: 1, buffer: buf2.buffer, palette: pal_bb },
    x,
    y
  );
  buf2.clear();
...
function drawN() {
  ...
  if (previous.wp_name !== wp.name || previous.selected !== selected) {
    ...
    if (selected) flip2_bw(160, 200);
    else flip2_bb(160, 200);
  }
}
...
function doselect() {
  if (
    selected &&
    wpindex != 0 &&
    waypoints[wpindex].lat === undefined &&
    savedfix.fix
  ) {
    waypoints[wpindex] = {
      name: "@" + wp.name,
