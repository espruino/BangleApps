...
function drawSteps(i) {
  if (!showSteps) return;
  if (i == undefined) i = lastStepsRowIndex;
...
function drawHeartRate(i) {
  ...
  if (i == undefined) i = lastHeartRateRowIndex;
  writeLineTopic("HRTM", i);
  if (hrtValue != undefined) {
    if (!hrtValueIsOld) writeLine(hrtValue, i);
    else writeLine(hrtValue, i, topicColor);
  }
...
function writeLine(str, line, pColor) {
  if (pColor == undefined) pColor = textColor;
  var y = marginTop + line * fontheight;
  g.setFont(font, fontsize);
...
Bangle.on("lock", function (isLocked) {
  if (!isLocked) {
    if (showHeartRate) {
      ...
      if (hrtValue == undefined) hrtValue = "...";
      else hrtValueIsOld = true;
    }
  } else {
    if (showHeartRate) {
      hrtValueIsOld = true;
