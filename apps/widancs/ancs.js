(() => {
  var s = require("Storage").readJSON("widancs.json", 1) || {
    settings: { enabled: false, category: [1, 2, 4] },
  };
  ...
  function advert() {
    NRF.setAdvertising(
      ...
      { connectable: true, discoverable: true, interval: 375 }
    );
  }
  ...
  if (
    !NRF.getGattforCentralServer &&
    ENABLED &&
    typeof SCREENACCESS != "undefined"
  )
  ...
  if (ENABLED && typeof SCREENACCESS != "undefined")
    NRF.on("connect", function (addr) {
      ...
      else NRF.connect(addr).then(do_bond);
    });
  ...
  function do_ancs() {
    ...
    state.gatt
      .getPrimaryService("7905F431-B5CE-4E99-A40F-4B1E122D00D0")
      .then(function (s) {
      ...
      .then(function (c) {
        state.ancs.notify = c;
        return state.ancs.primary.getCharacteristic(
  ...
  function wordwrap(s) {
    ...
    for (var i = 0; i < txt.length; i++) {
      ...
      if (l.length > MAXCHARS) {
        var p = MAXCHARS;
        while (p > MAXCHARS - 8 && !" \t-_".includes(l[p])) p--;
  ...
  function getnotify(d) {
    ...
    if (ct == 1) {
      // it's a call so pre-empt
      if (inalert) {
        state.notqueue.push(state.current);
        inalert = false;
      }
      state.notqueue.push({ cat: ct, uid: id });
    } else if (len < 16) state.notqueue[len] = { cat: ct, uid: id };
  ...
  function draw() {
    var colors = new Uint16Array([
      0xc618, 0xf818, 0x3ff, 0xffe0, 0x07e0, 0x0000,
    ]);
    var img = E.toArrayBuffer(
      atob(
        "GBgBAAAABAAADgAAHwAAPwAAf4AAP4AAP4AAP4AAHwAAH4AAD8AAB+AAA/AAAfgAAf3gAH/4AD/8AB/+AA/8AAf4AAHwAAAgAAAA"
      )
    );
