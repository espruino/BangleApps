...
function drawFinnFace() {
  ...
  g.fillRect(2, 100, 168, 180); // Squared hood bottom (x1, y1, x2, y2)
  
  // White hood ears
  g.setColor(0xFFFFFF);
  g.fillCircle(30, 20, 17); // Left ear (x, y, radius)
  g.fillCircle(140, 20, 17); // Right ear (x, y, radius)
  ...
  g.fillCircle(85, 80, 70); // Face circle (x, y, radius)
  
  // Outlines
  g.setColor(0x000000);
  g.drawCircle(85, 82, 70); // Face outline
  ...
  if (isCharging) {
    ...
    g.setColor(0x000000);
    g.fillCircle(32, 55, 10);  // Left eye (x, y, radius)
    g.fillCircle(139, 55, 10); // Right eye (x, y, radius)
  }
...
function drawJakeFace() {
  ...
  g.fillEllipse(130, 120, 45, 65); // Main jowl oval (center x, center y, width, height)
...
function draw() {
  ...
  if (character === "Finn") {
    g.setColor(0.5, 0.7, 1.0); // Light blue for Finn
  } else if (character === "Jake") {
...
function drawLockedScreen() {
  ...
  if (isCharging) {
    ...
    if (character === "Jake") {
      ...
      g.fillRect(40, 60, 60, 63); // left slit
      g.fillRect(110, 60, 130, 63); // right slit
    } else {
  ...
  // Mouth slit centered (always horizontal when locked)
  g.setColor(0x000000);
