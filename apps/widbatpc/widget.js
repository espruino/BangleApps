(function () {
  ...
  function draw() {
    ...
    if (Bangle.isCharging() && setting("charger")) {
      g.setColor(chargerColor()).drawImage(
    ...
    if (setting("fillbar")) xl = x + 4 + (100 * (s - 12)) / 100;
    ...
    if (setting("color") === "Monochrome") {
      ...
      gfx = Graphics.createCallback(g.getWidth(), g.getHeight(), 1, (x, y) => {
  ...
  Bangle.on("lcdPower", function (on) {
    if (on) {
      ...
      if (!batteryInterval) batteryInterval = setInterval(update, 60000);
    } else {
      if (batteryInterval) {
