...
function renderCoordinateSystem() {
  ...
  g.drawString(
    "30",
    GraphXMax + 2 * GraphMarkerOffset,
    GraphYZero - 50 - GraphMarkerOffset
  );
...
function loadLinesFromFile(requestedLineCount, fileName) {
  ...
  for (
    ...
    i < linesToReadCount + startingLineIndex;
    i++
  ) {
    if (allLines[i]) {
...
function renderData(dataArray) {
  ...
  var allConsumers =
    switchableConsumers.none |
    switchableConsumers.lcd |
    switchableConsumers.compass |
    switchableConsumers.bluetooth |
    switchableConsumers.gps |
    switchableConsumers.hrm;
  ...
  for (let i = 0; i < dataArray.length; i++) {
    ...
    let scaledTemp =
      Math.floor((datapointTemp * 100 - 2000) / 20) +
      ((datapointTemp * 100 - 2000) % 100) / 25;
