...
function draw() {
  ...
  g.drawString(stp, 3*w/4, ha);

  // time
  // white on red for battery warning
  g.setColor(!batWarn ? g.theme.bg : '#f00');
  g.fillRect(0, h2, w, h3);

  g.setFontLECO1976Regular42();
  g.setFontAlign(0, -1);
  g.setColor(!batWarn ? g.theme.fg : '#fff');
  ...
  g.drawString((stp / 1000 * settings.avStep).toFixed(2) + ' KM', w/2, ha + 107);
}
