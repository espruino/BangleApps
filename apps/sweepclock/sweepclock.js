...
class ThinHand extends Hand {
   ...
   * to draw.
   */
  constructor(centerX, centerY, length, tolerance, draw_test, color_theme) {
    super();
  ...
  moveTo(angle) {
    ...
    // for a redraw
    if (
      Math.abs(angle - this.angle) > this.tolerance ||
      ...
      var hand_color = default_white(
        color_schemes[color_scheme_index][this.color_theme]
      );
...
class ThickHand extends Hand {
  ...
  constructor(
    ...
    this.vertex_radius_base = Math.sqrt(
      (thickness * thickness) / 4 + base_height * base_height
    );
    // the radius that the top corners of the rectangle move through
    this.vertex_radius_top = Math.sqrt(
      (thickness * thickness) / 4 + length * length
    );
  ...
  moveTo(angle) {
    if (
      ...
      var y1 =
        this.centerY -
        this.vertex_radius_base * Math.cos(angle - this.delta_base);
      // bottom right
      var x2 =
        this.centerX +
        this.vertex_radius_base * Math.sin(angle + this.delta_base);
      ...
      var x3 =
        this.centerX +
        this.vertex_radius_top * Math.sin(angle + this.delta_top);
      var y3 =
        this.centerY -
        this.vertex_radius_top * Math.cos(angle + this.delta_top);
      // top left
      var x4 =
        this.centerX +
        this.vertex_radius_top * Math.sin(angle - this.delta_top);
...
let hour_hand_redraw = function (angle_from, angle_to, last_draw_time) {
  return (
    force_redraw ||
    (seconds_hand.angle >= angle_from &&
      seconds_hand.angle <= angle_to &&
      new Date().getTime() - last_draw_time.getTime() > 500)
  );
};
let hours_hand = new ThickHand(
  screen_center_x,
  screen_center_y,
  40,
  TWO_PI / 600,
...
function draw_date(date) {
  if (
    force_redraw ||
    last_date == null ||
    last_date.getDate() != date.getDate()
  ) {
    ...
    if (coords != null) {
      ...
      var numeral_color = default_white(
        color_schemes[color_scheme_index].numeral
      );
...
class HourScriber {
  ...
  drawHour(hours) {
    ...
    if (
      ...
      if (
        Math.min(angle1, angle2, angle3, angle4) < Math.PI &&
        Math.max(angle1, angle2, angle3, angle4) > 1.5 * Math.PI
      ) {
        angle1 = rebaseNegative(angle1);
        angle2 = rebaseNegative(angle2);
        angle3 = rebaseNegative(angle3);
        angle3 = rebaseNegative(angle4);
        this.angle_from = rebasePositive(
          Math.min(angle1, angle2, angle3, angle4)
        );
        this.angle_to = rebasePositive(
          Math.max(angle1, angle2, angle3, angle4)
        );
      } else {
...
let hour_scriber = new HourScriber(
  70,
...
function draw_background() {
  if (force_redraw) {
    var background = color_schemes[color_scheme_index].background;
    g.setColor(background[0], background[1], background[2]);
    g.fillPoly([0, 25, 0, 240, 240, 240, 240, 25]);
  }
}
