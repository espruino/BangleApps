...
var csv = ["time", "sample count", "HR", "SDNN", "RMSSD", "Temp", "movement"];
...
function rolling_average(values, count) {
  ...
  for (let i = 0; i < values.length; i++) {
    temp_array = [];
    for (let x = 0; x < count; x++) temp_array.push(values[i + x]);
...
function apply_cutoff() {
  ...
  for (let i = 0; i < alternate_array.length; i++) {
    x = alternate_array[i];
    if (x < cutoff_threshold) x = cutoff_threshold;
    alternate_array[i] = x;
  }
}
...
function calculate_HRV() {
  ...
  if (option == 0) Bangle.setLCDPower(1);
  g.clear();
...
Bangle.on("HRM-raw", function (e) {
  ...
  if (!(samples & 7)) {
    ...
    if (samples < 100)
      ...
      g.drawString(
        "logging: " + ((samples * 100) / raw_HR_array.length).toFixed(0) + "%",
        px,
        py,
        true
      );
