E.showMenu = function (items) {
  ...
  var l = {
    ...
    draw: function () {
      ...
      if (lw + 2 >= w) {
        // label width doesn't fit at large size: scale it down
        g.setFont("Vector", Math.floor((large * ar.w) / lw));
      }
      g.clearRect(x, vc - ih / 2, x2, vc + ih / 2)
        .setFontAlign(0, 0, 0)
        .drawString(item.lbl, hc, vc);

      if (item.v !== undefined) {
        g.setColor(g.theme.fgH)
          .setBgColor(g.theme.bgH) // always highlighted: either as part of item, or while editing
          .setFontAlign(0, 1, 0)
      ...
      if (l.lastIdx !== idx) {
        ...
        g.clearRect(x, y, x2, vc - ih / 2 - 1).clearRect(
          x,
          vc + ih / 2 + 1,
          x2,
          y2
        ...
        g.setFontAlign(0, 1);
        if (wrap || idx > 0)
          g.setFont("Vector", small).drawString(
            lbl(idx - 1),
            hc,
            vc - ih / 2 - 5
          );
        if (wrap || idx > 1)
          g.setFont("Vector", small / 2).drawString(
            lbl(idx - 2),
            hc,
            vc - ih / 2 - small - 10
          );
        // next two items
        g.setFontAlign(0, -1);
        if (wrap || idx < menuItems.length - 1)
          g.setFont("Vector", small).drawString(
            lbl(idx + 1),
            hc,
            vc + ih / 2 + 5
          );
        if (wrap || idx < menuItems.length - 2)
          g.setFont("Vector", small / 2).drawString(
            lbl(idx + 2),
            hc,
            vc + ih / 2 + small + 10
        ...
        if (back) {
          g.setBgColor(g.theme.bg2).setFont("Vector", small);
          ...
          g.setColor(g.theme.bg2)
            ...
            .drawPoly(poly, true)
            .setFontAlign(-1, -1, 0)
            .drawString(backLbl, x + 2, y + 2);
        }
      }
      g.flip();
    },
    select: function () {
      // same as default menu
    ...
    move: function (dir) {
      if (l.selectEdit) {
        ...
        if (
          !wrap &&
          (options.selected + dir < 0 ||
            options.selected + dir >= menuItems.length)
        ) {
          return;
        }
        options.selected =
          (options.selected + dir + menuItems.length) % menuItems.length;
      }
      l.draw();
    },
  };
