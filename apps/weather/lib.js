...
function update(weatherEvent) {
  ...
  if (weatherEvent) {
    ...
    if (weather.wdir != null) {
      ...
      weather.wrose = ["n", "ne", "e", "se", "s", "sw", "w", "nw", "n"][
        Math.floor((deg + 22.5) / 45)
      ];
    }
  ...
  function drawSnow(x, y, r) {
    function rotatePoints(points, pivotX, pivotY, angle) {
      for (let i = 0; i < points.length; i += 2) {
        ...
        const y = points[i + 1];
        points[i] =
          Math.cos(angle) * (x - pivotX) -
          Math.sin(angle) * (y - pivotY) +
          pivotX;
        points[i + 1] =
          Math.sin(angle) * (x - pivotX) +
          Math.cos(angle) * (y - pivotY) +
          pivotY;
      }
    }
    ...
    const w = (1 / 12) * r;
    for (let i = 0; i <= 6; ++i) {
      const points = [x + w, y, x - w, y, x - w, y + r, x + w, y + r];
