...
function loadSettings() {
  ...
  if (changed) require("Storage").writeJSON("recorder.json", settings);
}
loadSettings();
...
function viewTracks() {
  ...
  require("Storage")
    ...
    .forEach((filename) => {
      found = true;
      menu["Track " + getTrackNumber(filename)] = () =>
        viewTrack(filename, false);
    });
  if (!found) menu["No Tracks found"] = function () {};
  menu["< Back"] = () => {
    showMainMenu();
  };
...
function plotTrack(info) {
  ...
  if (!info.qOSTM) {
    g.setColor("#f00")
      .fillRect(9, 80, 11, 120)
      .fillPoly([9, 60, 19, 80, 0, 80]);
