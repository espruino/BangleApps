...
function drawButton(x, y, state) {
  ...
  else g.fillCircle(x + button_h / 2, y + button_h / 2, button_r);
  g.flip();
}
...
function setup_screen() {
  ...
  for (var i = 0; i < 4; ++i) {
    g.setFontVector(22)
      .setFontAlign(-1, 0, 0)
      .setColor(0xffff)
      .drawString("Ch" + String(i + 1), 16, 60 + i * button_sp + button_h / 2);
    drawButton(
      (g.getWidth() - button_w) / 2,
      60 + i * button_sp,
      channel_state[i]
    );
  }
...
function parseDevice(d) {
  ...
  g.clearRect(0, 60, 239, 239)
    .setFontAlign(0, 0, 0)
    .setColor(0, 1, 0)
    .drawString("Found device", 120, 120)
    .flip();
  device.gatt
    .connect()
    .then(function (ga) {
...
function moveChannelFrame(oldc, newc) {
  g.setColor(0).drawRect(
    8,
    60 + oldc * button_sp - 4,
    g.getWidth() - 8,
    60 + oldc * button_sp + button_h + 4
  );
  g.setColor(0.9, 0.9, 0.9).drawRect(
    8,
    60 + newc * button_sp - 4,
    g.getWidth() - 8,
    60 + newc * button_sp + button_h + 4
  );
}
...
function setup_app() {
  characteristic
    .readValue()
    .then(function (r) {
      for (var i = 0; i < r.buffer.length / 4; ++i)
        channel_state.push(r.buffer[i * 4 + 2] == 1);
    })
    .then(setup_screen);
