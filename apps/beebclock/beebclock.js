...
Graphics.prototype.fillRotRect = function (sina, cosa, cx, cy, x0, x1, y0, y1) {
  ...
  return this.fillPoly([
    fn(cx - x0 * cosa + y0 * sina),
    fn(cy - x0 * sina - y0 * cosa),
    fn(cx - x1 * cosa + y0 * sina),
    fn(cy - x1 * sina - y0 * cosa),
    fn(cx - x1 * cosa + y1 * sina),
    fn(cy - x1 * sina - y1 * cosa),
    fn(cx - x0 * cosa + y1 * sina),
    fn(cy - x0 * sina - y1 * cosa),
  ]);
};
...
const drawAll = (force) => {
  ...
  tmp = hours;
  hours = now.getHours();
  if (tmp !== hours) face_changed = true;

  tmp = minutes;
  minutes = now.getMinutes();
  if (tmp !== minutes) face_changed = true;
  ...
  if (time_changed) {
    ...
    if (with_digital_date || with_digital_time) {
      ...
      if (with_digital_time) {
        txt = da[4].substr(0, 5);
        if (with_digital_date) G1.drawStringDH(txt + ",", 24, 0, "L", GW);
        else G1.drawStringDH(txt, 0, 0, "C", GW);
      }

      if (with_digital_date) {
        let txt = [da[0], da[1], da[2]].join(" ");
        if (with_digital_time) G1.drawStringDH(txt, -24, 0, "R", GW);
        else G1.drawStringDH(txt, 0, 0, "C", GW);
      }
    }
    ...
    // image to the screen.
    g.setColor(1, 1, 1);
    g.drawImage({ width: GW, height: GH - TM, buffer: G1.buffer }, 0, TM);
    ...
    if (with_widgets) Bangle.drawWidgets();
  }
  ...
  if (with_seconds) inOneSecond(drawAll);
  else inOneMinute(drawAll);
};
