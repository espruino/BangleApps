...
let drawInfo = function() {
  ...
  g.drawString(/*LANG*/"Powered by Espruino",x,y+=4+h);
  g.drawString(/*LANG*/"Version "+ENV.VERSION,x,y+=h);
  g.drawString("Commit "+ENV.GIT_COMMIT,x,y+=h);

  getVersion("Bootloader","boot.info");
  getVersion("Launcher","launch.info");
  getVersion("Settings","setting.info");

  g.drawString(MEM.total+/*LANG*/" JS Vars",x,y+=h);
  g.drawString("Storage: "+(require("Storage").getFree()>>10)+/*LANG*/"k free",x,y+=h);
  if (ENV.STORAGE) g.drawString("         "+(ENV.STORAGE>>10)+/*LANG*/"k total",x,y+=h);
  if (ENV.SPIFLASH) g.drawString("SPI Flash: "+(ENV.SPIFLASH>>10)+"k",x,y+=h);
  imgTop = y+h;
  imgScroll = imgHeight-imgTop;
  g.reset().setFont("6x15").setFontAlign(0,0);
  g.drawString(ENV.VERSION + "  " + NRF.getAddress(), W/2, imgMaxY+9);
