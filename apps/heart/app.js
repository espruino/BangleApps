...
function log(msg) {
  console.log(
    "heart: " +
      msg +
      "; mem used: " +
      process.memory().usage / process.memory().blocksize
  );
  return;
}
...
function updateSettings() {
  ...
  if (WIDGETS["heart"]) WIDGETS["heart"].reload();
  return;
}
    ...
    if (line !== undefined) {
      menu[
        "#" +
          n +
          " " +
          getDateString(line.split(",")[0] * 1000) +
          " " +
          getTimeString(line.split(",")[0] * 1000)
      ] = func.bind(null, n);
      found = true;
    }
  }
  if (!found) menu["No Records Found"] = function () {};
  menu["< Back"] = () => {
    showMainMenu();
  };
...
function viewRecord(n) {
  ...
  var count = 0;
  var value = 0;
  if (l !== undefined) heartTime = new Date(l.split(",")[0] * 1000);
  log("parsing records");
  while (l !== undefined) {
    count++;
...
function graphRecord(n) {
  ...
  if (lineCount > MaxValueCount) startLine = lineCount - MaxValueCount;
  f = undefined;
  line = undefined;
  lineCount = undefined;
  ...
  while (line !== undefined) {
    ...
    if (tempCount == startLine) {
      ...
      g.reset()
        .clearRect(0, 24, g.getWidth(), g.getHeight())
        // Home for Btn2
        .setColor(g.theme.fg)
        ...
        .drawString(
          headline,
          g.getWidth() / 2 - headline.length / 2,
          GraphY100 - g.getFontHeight() - GraphMarkerOffset
        )
        // Chart
        .setColor(1, 1, 0)
        ...
        .drawLine(
          GraphXZero,
          GraphYZero + GraphMarkerOffset,
          GraphXZero,
          GraphY100
        )
        // vertical left line
        .drawLine(
          GraphXZero - GraphMarkerOffset,
          GraphYZero,
          GraphXMax + GraphMarkerOffset,
          GraphYZero
        )
        ...
        .drawLine(
          GraphXZero - GraphMarkerOffset,
          GraphY100 + (GraphYZero - GraphY100) / 2,
          GraphXZero,
          GraphY100 + (GraphYZero - GraphY100) / 2
        )
      ...
      for (var i = MaxMeasurement; i >= MinMeasurement; i -= 10) {
        g.drawString(
          i,
          GraphXLabel,
          GraphY100 + LabelOffset * (MaxMeasurement - i) - GraphMarkerOffset
        );
      }
  ...
  if (times[1] !== 0) {
    ...
    g.setFontAlign(-1, -1, 0)
      .drawString(
        dateStr,
        GraphXMax / 2 - dateStr.length / 2 - GraphMarkerOffset,
        GraphYZero + 12
      )
      .setFontAlign(1, -1, 0)
