...
const ulCornerTextXScreen =
  ulCornerX + parseInt(((noOfSquX - textWidth) * bigSquareSize) / 2);
const ulCornerTextYScreen =
  ulCornerY + parseInt(((noOfSquY - textHeight) * bigSquareSize) / 2);
const seculCornerTextX = ulCornerTextX + textWidth - sectextWidth;

let buf = Graphics.createArrayBuffer(screenWidth, screenHeight, 1, {
  msb: true,
});
let textbufMin = Graphics.createArrayBuffer(
  (textWidth - sectextWidth) * bigSquareSize,
  textHeight * bigSquareSize,
  1,
  { msb: true }
);
let textPreBufMin = Graphics.createArrayBuffer(
  textWidth - sectextWidth,
  textHeight,
  8,
  { msb: true }
);
let textbufSec = Graphics.createArrayBuffer(
  sectextWidth * bigSquareSize,
  textHeight * bigSquareSize,
  1,
  { msb: true }
);
let textPreBufSec = Graphics.createArrayBuffer(sectextWidth, textHeight, 8, {
  msb: true,
});
...
function renderSecText() {
  g.clearRect(
    ulCornerTextXScreen + 18 * bigSquareSize + padding,
    ulCornerTextYScreen,
...
function flip(dontclear) {
  g.setColor(1, 1, 1);
  g.drawImage(
    { width: screenWidth, height: screenHeight, bpp: 1, buffer: buf.buffer },
    ulCornerX,
    ulCornerY
  );
...
function skipCell(x, y) {
  return (
    (x >= seculCornerTextX &&
      x < seculCornerTextX + sectextWidth &&
      y >= ulCornerTextY &&
      y < ulCornerTextY + textHeight) ||
    (generation < genEnd &&
      x >= ulCornerTextX &&
      x < ulCornerTextX + textWidth &&
      y >= ulCornerTextY &&
      y < ulCornerTextY + textHeight)
  );
}
...
function updateSecondText() {
  ...
  if (
    !lastDate ||
    (generation < genEnd && lastDate.getMinutes() != startDate.getMinutes())
  ) {
