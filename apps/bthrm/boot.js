(function () {
  ...
  Bangle.isHRMOn = function () {
    return (
      status == "searching" || status == "connecting" || gatt !== undefined
    );
  };
  Bangle.setHRMPower = function (isOn, app) {
    ...
    if (!isOn && Bangle._PWR.HRM.includes(app))
      Bangle._PWR.HRM = Bangle._PWR.HRM.filter((a) => a != app);
    ...
    if (isOn) {
      ...
      if (!Bangle.isHRMOn()) {
        ...
        status = "searching";
        NRF.requestDevice({ filters: [{ services: ["180D"] }] })
          ...
          .then(function (characteristic) {
            ...
            characteristic.on("characteristicvaluechanged", function (event) {
              ...
              // 4 = RR interval
              var bpm =
                flags & 1 ? dv.getUint16(1) / 100 /* ? */ : dv.getUint8(1); // 8 or 16 bit
