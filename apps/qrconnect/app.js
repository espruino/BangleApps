...
int get_qr (char *string, uint8_t *qrcodeBitmap) {
    ...
    for (uint8_t y = 0; y < qrcode.size; y++) {
        for (uint8_t x = 0; x < qrcode.size; x++) {
            int bit = x + y * qrcode.size;
            qrcodeBitmap[bit>>3] |= qrcode_getModule(&qrcode, x, y)<<(7-(bit&7));
        }
    }
    return qrcode.size;
}
...
function getBinaryQR (value) {
  var qrcodeBitmap = new Uint8Array(106);
...
let qr = getBinaryQR("https://banglejs.com/apps/?dev="+deviceName.replace(" ","%20"));
let bmp = Graphics.createArrayBuffer(qr.size,qr.size,1);
(new Uint8Array(bmp.buffer)).set(qr.data);
// render the QR code and device name
var fontHeight = 17;
var scale = Math.floor((g.getHeight()-fontHeight)/(qr.size+2));
