...
var img = require("heatshrink").decompress(
  atob(
...
function draw() {
  ...
  if (E.getBattery() < 15) batteryWarning = true;

  // turn the warning off once we have dipped above 20%
  if (E.getBattery() > 20) batteryWarning = false;
  ...
  // day and steps
  if (settings.color == "Blue" || settings.color == "Red") g.setColor("#fff");
  // white on blue or red best contrast
  else g.setColor("#000"); // otherwise black regardless of theme
  ...
  g.drawString(
    ((stps / 1000) * settings.avStep).toFixed(2) + " KM",
    w / 2,
    ha + 107
  );
}
