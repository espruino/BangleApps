...
function showData() {
  ...
  g.setFontAlign(0, 0);
  for (var l = -8; l <= 8; l++) g.drawString(l, 5, m - l * s);
  ...
  g.drawString(
    "Max Y Accel: " + maxAccel.toFixed(2) + " g",
    g.getWidth() - 14,
    g.getHeight() - 50
  );
  g.drawString(
    "Max Y Vel: " + maxVel.toFixed(2) + " m/s",
    g.getWidth() - 14,
    g.getHeight() - 40
  );
  g.drawString(
    "Time moving: " + (tEnd - tStart) / HZ + " s",
    g.getWidth() - 14,
    g.getHeight() - 30
  );
...
function showSaveMenu() {
  ...
  [1, 2, 3, 4, 5, 6].forEach((i) => {
    ...
    menu["Recording " + i + (exists ? " *" : "")] = function () {
      var csv = "";
      for (var i = 0; i < SAMPLES; i++)
        csv += `${accelx[i] / SCALE},${accely[i] / SCALE},${
          accelz[i] / SCALE
        }\n`;
