...
let hour_numeral_redraw = function (angle_from, angle_to, last_draw_time) {
  ...
  var redraw =
    force_redraw ||
    (seconds_hand_angle >= angle_from &&
      seconds_hand_angle <= angle_to &&
      seconds_hand.last_draw_time.getTime() > last_draw_time.getTime()) ||
...
var numeral_colour_supplier = () =>
  default_white(color_schemes[color_scheme_index].numeral);
var hour_scribers = [];
...
function next_font() {
  ...
  if (curr_font != numeral_fonts_index) {
    ...
    for (var i = 0; i < hour_scribers.length; i++) {
      hour_scribers[i].setNumeralFont(numeral_fonts[numeral_fonts_index]);
    }
    force_redraw = true;
    return true;
  } else {
...
function draw_hour_digits() {
  if (force_redraw) {
    ...
    if (hour_scriber_idx >= hour_scribers.length) hour_scriber_idx = 0;
