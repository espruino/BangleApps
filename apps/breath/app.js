...
if (test !== undefined) settings = JSON.parse(test);
...
function circle() {
  ...
  if (settings.vibrate == "forward") Bangle.buzz(50, Math.abs(origin) / 1.5);
...
function update_menu() {
  ...
  while (cursor < 180) {
    ...
    } else g.setColor(settings.colour[0]);
...
function btn3Pressed() {
  if (status < 6) {
    if (status == 0) {
      settings.period += 1;
      if (settings.period > 6) settings.period = 1;
    } else if (status == 1) {
      settings.exhale_pause += 1;
      if (settings.exhale_pause > 4) settings.exhale_pause = 1;
    } else if (status == 2) {
      settings.inhale_pause += 1;
      if (settings.inhale_pause > 4) settings.inhale_pause = 1;
    } else if (status == 3) {
      ...
      if (settings.vibrate == "forward") settings.vibrate = "backward";
      else if (settings.vibrate == "backward") settings.vibrate = "off";
      else if (settings.vibrate == "off") settings.vibrate = "forward";
    } else if (status == 5) {
      if (settings.ex_in_ratio == "1:1") settings.ex_in_ratio = "5:6";
      else settings.ex_in_ratio = "1:1";
    }
    update_menu();
  }
}
...
Bangle.on("HRM", function (hrm) {
  if (first_signal) first_signal = false;
  else {
    var signal = hrm.bpm;
    if (signal > 50 && signal < 180) display_HR = signal;
  }
});
