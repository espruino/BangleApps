...
const GraphicUtils = {
  ...
  draw_cosine: (from_x, to_x, line_colour, screen_info, img_info) => {
    ...
    for (var x = from_x; x < to_x; x++) {
      var radians =
        (Math2.TWO_PI *
          (x - screen_info.screen_start_x - screen_info.screen_centre_x)) /
        screen_info.screen_width;
      var y =
        screen_info.screen_centre_y -
        (screen_info.screen_height * Math.cos(radians)) / 2;
      if (first) {
  ...
  draw_sunrise_line: (sunrise_colour, day_info, screen_info, img_info) => {
    ...
    if (rise_y == null && day_info != null) {
      var rise_fraction =
        (day_info.sunrise_date.getTime() - day_info.day_start.getTime()) /
        DateUtils.DAY_MILLIS;
      var rise_radians = Math2.TWO_PI * (rise_fraction - 0.5);
      var rise_y =
        screen_info.screen_centre_y -
        (screen_info.screen_height * Math.cos(rise_radians)) / 2;
      //console.log("rise_y=" + rise_y + " rise_fraction=" + rise_fraction + " rise_radian=" + rise_radians);
      screen_info.sunrise_y = rise_y;
    }
    if (rise_y != null) {
      var draw_info = _draw_info(img_info);
      draw_info.buff.setColor(
        sunrise_colour[0],
        sunrise_colour[1],
        sunrise_colour[2]
      );
      draw_info.buff.drawLine(
        screen_info.screen_start_x - draw_info.offset_x,
  ...
  fill_circle_partial_y: (
    ...
    for (var y = from_y; y <= to_y; y++) {
      ...
      if (x_dist2 >= 0) {
        ...
        if (to_x - to_x_rounded > 0.01) to_x_rounded += 1;

        draw_info.buff.drawLine(
          from_x - draw_info.offset_x,
