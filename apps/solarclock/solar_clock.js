...
function draw_sun(now, day_info) {
  var now_fraction =
    (now.getTime() - day_info.day_start.getTime()) / DateUtils.DAY_MILLIS;
  var now_x = now_fraction * screen_info.screen_width;
  if (screen_info.sun_x != null && Math.abs(now_x - screen_info.sun_x) < 1) {
    console.log(
      "no sun movement now_x:" + now_x + " screen sun_x:" + screen_info.sun_x
    );
    return false;
  }
  // now calculate thew new sun coordinates
  var now_radians =
    (Math2.TWO_PI * (now_x - screen_info.screen_centre_x)) /
    screen_info.screen_width;
  var now_y =
    screen_info.screen_centre_y -
    (screen_info.screen_height * Math.cos(now_radians)) / 2;
  if (screen_info.sun_x != null && Math.abs(now_x - screen_info.sun_x) > 5.0) {
    clear_sun();
  } else if (
    screen_info.sun_y != null &&
    Math.abs(now_y - screen_info.sun_y) > 5.0
  ) {
    clear_sun();
  }
  ...
  if (draw_full_cosine) {
    ...
    GraphicUtils.draw_cosine(
      screen_info.screen_start_x,
      screen_info.screen_width,
      COSINE_COLOUR,
      screen_info
    );
  ...
  if (sunrise_dist <= img_height) {
    GraphicUtils.draw_sunrise_line(
      HORIZON_COLOUR,
      day_info,
      screen_info,
      img_info
  ...
  g.fillRect(
    img_info.x,
    img_info.y - 2,
    img_info.x + img_width,
    img_info.y + img_height + 2
  );
...
function write_GPS_status() {
  if (!gps_status_requires_update) return;
  ...
  if (gps_coords_msg == null) {
    gps_coords_msg = [
      "N:" + Math2.format000_00(gps_coords[1]),
...
function write_twilight_times() {
  if (!twilight_times_requires_update) return;

  var twilight_msg;
  if (day_info != null) {
    twilight_msg = [
      format_time(day_info.sunrise_date),
      format_time(day_info.sunset_date),
    ];
  } else {
...
function format_offset() {
  ...
  var sign = "+";
  if (time_offset < 0) sign = "-";

  return (
    sign +
    Math2.format00(hours_offset) +
    ":" +
    Math2.format00(mins_offset_from_hour)
  );
}
...
function write_location_name() {
  if (!location_requires_update) return;
...
location.addUpdateListener((loc) => {
  console.log("location update:" + JSON.stringify(loc));
  clear_sun();
  GraphicUtils.draw_sunrise_line(
    screen_info.screen_bg_color,
    day_info,
    screen_info
  );
  day_info = null;
...
function dayInfo(now) {
  if (day_info == null || now > day_info.day_end || now < day_info.day_start) {
    ...
    if (coords != null) {
      day_info = DateUtils.sunrise_sunset(
        now,
        coords[0],
        coords[1],
        location.getUTCOffset()
      );
      twilight_times_requires_update = true;
...
function draw_clock() {
  ...
  console.log(
    "drawing clock:" + now.toISOString() + " time taken:" + time_taken
  );
}
