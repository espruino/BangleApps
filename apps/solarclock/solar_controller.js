...
function daytime_sun_color(now, day_info) {
  ...
  if (now > day_info.sunset_date) {
    ...
    var sunrise_fraction =
      (day_info.sunrise_date - day_info.day_start) / DateUtils.DAY_MILLIS;
    var rise_to_midday_fraction =
      (now_fraction_of_day - sunrise_fraction) / (0.5 - sunrise_fraction);
    return Math2.interpolate(
      SUNRISE_COLOUR,
      MIDDAY_COLOUR,
      rise_to_midday_fraction
    );
  } else {
    var sunset_fraction =
      (day_info.day_end - day_info.sunset_date) / DateUtils.DAY_MILLIS;
    var midday_to_sunset_fraction =
      (now_fraction_of_day - 0.5) / (0.5 - sunset_fraction);
    //console.log("sunset_fraction=" + sunset_fraction + " midday_to_sunset_fraction=" + midday_to_sunset_fraction)
    return Math2.interpolate(
      MIDDAY_COLOUR,
      SUNSET_COLOUR,
      midday_to_sunset_fraction
    );
  }
}
...
function draw_night_sun(sun_x, sun_y, sun_radius, img_info) {
  ...
  draw_info.buff.fillCircle(
    sun_x - draw_info.offset_x,
...
function draw_partial_sun(time, day_info, screen_info, img_info) {
  ...
  if (sun_height > screen_info.sun_radius) {
    ...
    draw_info.buff.fillCircle(
      screen_info.sun_x - draw_info.offset_x,
    ...
    var sun_color = daytime_sun_color(time, day_info);
    GraphicUtils.set_color(sun_color, draw_info.buff);
    draw_info.buff.drawCircle(
      screen_info.sun_x - draw_info.offset_x,
      screen_info.sun_y - draw_info.offset_y,
      screen_info.sun_radius
    );
    GraphicUtils.fill_circle_partial_y(
      screen_info.sun_x,
      screen_info.sun_y,
      screen_info.sun_radius,
...
function draw_random_background(screen_info, img_info, rgb_init, rgb_step) {
  var draw_info = GraphicUtils.draw_info(img_info);
  var rgb = rgb_init;
  var sky_to = Math.min(
    screen_info.sunrise_y - 1,
    img_info.y + img_info.img.height - 3
  );
...
class DayNightMode extends SolarMode {
  ...
  _calc_corona_radius(now, day_info) {
    if (now < day_info.sunset_date && now > day_info.sunrise_date) {
      var now_fraction_of_day = DateUtils.now_fraction_of_day(now, day_info);
      var sunset_fraction =
        (day_info.sunset_date.getTime() - day_info.day_start.getTime()) /
    ...
    if (corona_radius > from_radius + CORONA_MIN_LENGTH) {
      for (var i = 0; i < CORONA_LINES; i++) {
        var to_x1 =
          ...
          draw_info.offset_x +
          from_radius *
            Math.cos((i * Math2.TWO_PI) / CORONA_LINES + thickness_rads);
        var to_y1 =
          sun_y -
          draw_info.offset_y +
          from_radius *
            Math.sin((i * Math2.TWO_PI) / CORONA_LINES + thickness_rads);
        var to_x2 =
          sun_x -
          draw_info.offset_x +
          from_radius *
        ...
        var to_x3 =
          sun_x -
          draw_info.offset_x +
          corona_radius * Math.cos((i * Math2.TWO_PI) / CORONA_LINES);
        var to_y3 =
          sun_y -
          draw_info.offset_y +
          corona_radius * Math.sin((i * Math2.TWO_PI) / CORONA_LINES);
...
class TwiLightMode extends SolarMode {
  test(time, day_info, screen_info) {
    ...
    if (
      sun_height > -screen_info.sun_radius &&
