...
if (test_file !== undefined) {
  ...
  if (astral_settings.astral_default) display_colour = default_colour;
  else display_colour = setupcomplete_colour;
}
...
function process_extras_coord(coord_string) {
  ...
  if (extras_signcheck == "-") {
    ...
    extras_calc =
      (extras_hour + extras_minute / 60 + extras_second / 3600) * -1;
  } else {
...
function mean_sidereal_time(lon) {
  ...
  // days since J2000.0
  var jd =
    b + c + da - 730550.5 + day + (hour + mins / 60.0 + secs / 3600.0) / 24.0;
...
function write_refresh_note(colour) {
  ...
  } else g.drawString("updating, please wait", Xaxis + 50, cursor, false);
}
...
function draw_moon(phase) {
  ...
  } else if (phase == 4) g.fillCircle(200, Yaxis, 30);
  else if (phase == 3) {
    g.fillCircle(200, Yaxis, 30);
    g.setColor("#000000");
...
function draw() {
  if (astral_settings.astral_default) display_colour = default_colour;
  else if (!colours_switched) display_colour = setupcomplete_colour;
  ...
  if (ready_to_compute) g.setColor(calc_display_colour);

  if (modeswitch) g.setColor("#000000");
  ...
  if (modeswitch)
    if (ready_to_compute) write_refresh_note(calc_display_colour);
    else write_refresh_note(display_colour);
...
Bangle.on("lcdPower", (on) => {
  ...
  if (!astral_settings.astral_default) Bangle.setGPSPower(0);
  if (on) {
...
Bangle.setUI("clockupdown", (btn) => {
  if (btn == 0) {
    if (!processing) {
      ...
      } else modeswitch = false;
    }
  } else {
    if (!processing) ready_to_compute = true;
  }
});
...
Bangle.on("mag", function (m) {
  g.setFont("6x8", 2);
  if (isNaN(m.heading)) compass_heading = "---";
  else compass_heading = 360 - Math.round(m.heading);
  //  g.setColor("#000000");
  // g.fillRect(160, 10, 160, 20);
  g.setColor(display_colour);
  if (compass_heading < 100) compass_heading = " " + compass_heading;
