(function() {
  ...
  // ===== SETTINGS =====
  const settings = storage.readJSON("tileclk.json", true) || {
    widgets: "show",
    seconds: "dynamic",
    borders: true,
    borderColor: "theme"
  };
  const systemSettings = storage.readJSON("setting.json", true) || {};
  const is12Hour = systemSettings["12hour"] || false;
  let showSeconds = settings.seconds === "show" || (settings.seconds === "dynamic" && !Bangle.isLocked());
  const showBorders = settings.borders !== false;
  const touchBuzzStrength = Bangle.haptic ? (((systemSettings.options || {}).hapticTime ?? 25) / 50) : null;
  ...
  let clockInfoMenu = null, clockInfoItems = null, touchHandler = null, lockHandler = null;
  ...
  const loadDisplayState = () => {
    const state = storage.readJSON("tileclk.state.json", true) || {};
  ...
  const saveDisplayState = () => {
    ...
    if (showingClockInfo !== initialShowingClockInfo ||
      ...
      storage.writeJSON("tileclk.state.json", {
  ...
  const buzzTouch = (ms) => {
    if (systemSettings.vibrate === false) return;
    if (touchBuzzStrength !== null) {
      if (!touchBuzzStrength) return;
      Bangle.buzz(ms, touchBuzzStrength);
      return;
    }
    Bangle.buzz(ms);
  }
  ...
  function updateSeconds() {
    ...
    if (skipAnimation) {
      ...
      const tiles0 = calculateTilesToUpdate(positions.seconds.x[0], positions.seconds.y + widgetYOffset, SEC_SCALE, s1, null);
      const tiles1 = calculateTilesToUpdate(positions.seconds.x[1], positions.seconds.y + widgetYOffset, SEC_SCALE, s2, null);
  ...
  const setClockInfoFocus = (focused, forceRedraw) => {
    clockInfoUnfocused = !focused;
    if (!clockInfoMenu) return;
    if (clockInfoMenu.setFocus) {
      if (clockInfoMenu.focus !== focused) {
        clockInfoMenu.setFocus(focused);
        return;
      }
    } else if (clockInfoMenu.focus !== focused) {
      clockInfoMenu.focus = focused;
    }
    if (forceRedraw && clockInfoMenu.redraw) {
      clockInfoMenu.redraw();
    }
  }

  const destroyClockInfoMenu = () => {
    if (!clockInfoMenu) return;
  ...
  const ensureClockInfo = () => {
    if (!clockInfoMenu) setupClockInfo();
    return !!clockInfoMenu;
  }

  // ===== SWITCHING FUNCTIONS (Optimized with direct state changes) =====
  const switchToClockInfo = () => {
    if (showingClockInfo || !ensureClockInfo()) {
      pendingSwitch = false;
      return;
    }
  ...
  const setupClockInfo = () => {
    ...
    try {
      ...
      const clockInfoDraw = (_, info, options) => {
        ...
        let text = info.text == null ? "" : String(info.text);
  ...
  const drawClock = () => {
    ...
    if ((showingClockInfo || userClockInfoPreference === 'show') && ensureClockInfo()) {
      // Restore saved clock info state without forcing focus on startup
      showingClockInfo = true;
      setClockInfoFocus(false, true);
    } else {
    ...
    if (settings.seconds === "dynamic") {
      ...
      if (isLocked) {
        if (!showingClockInfo && lastSeconds !== null) {
          if (secondsTimeout) clearTimeout(secondsTimeout);
          clearSeconds(() => {
            if (userClockInfoPreference === 'show' && ensureClockInfo()) {
              showingClockInfo = true;
              setClockInfoFocus(false, true);
            }
            g.flip();
