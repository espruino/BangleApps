...
* **Steps:** Tracks your daily steps using a sweeping dial. Tap to briefly show the exact step count.
* **Distance:** Shows the distance covered today, plus an optional trip meter: swipe up to start a trip ("count from here"), swipe down to return to the day total — the trip keeps running in the background. Tap to briefly show the exact distance of the current view. Swiping up in the trip view asks to reset the trip (a RESET? popup); confirm with one more swipe up, or dismiss it by swiping down or waiting. The trip view is marked with a TRIP indicator, survives app reloads, and automatically ends at midnight.
...
* **Tap (Steps / Distance):** Shows the exact step count or distance of the current view for a few seconds; tap again to dismiss it early.
