...
* **Timer:** A wind-up kitchen timer, one swipe up from the clock face, on a 0-60 minute dial (one revolution per hour). Hold a finger on the screen until the watch buzzes — you have "grabbed" the dial — then turn clockwise to add minutes, like winding an egg timer; letting go starts the countdown, and winding back to zero switches the timer off. The needle is magenta while the timer runs and red while it is paused or off; the circle shows the remaining minutes (seconds in the final minute) and a pause icon while paused. Swipe up to pause, swipe down to resume; swiping up on a paused timer asks to reset it (RESET? popup, confirm with one more swipe up). The ringing is handled by the system scheduler, so the timer fires with the standard alarm screen (buzz, snooze, dismiss) even if a message or another app has taken over the screen in the meantime — it even survives a reboot. A tap shows the exact remaining time.
