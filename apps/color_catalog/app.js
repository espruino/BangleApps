...
function UserInput() {
  ...
  Bangle.on("swipe", (dir) => {
    if (dir == 1) {
      console.log(
        "v_acolorpos" + v_acolorpos + "a_colors.length" + a_colors.length
      );
  ...
  for (cont_row = 0; cont_row < 2; cont_row++) {
    ...
    for (cont_items = 0; cont_items < v_boxes_row; cont_items++) {
      ...
      var v_pos_row =
        ...
        v_fontsize +
        5 +
        cont_row * v_boxheight +
        cont_row * v_boxes_row * (v_fontsize + 4);
      ...
      var v_string =
        v_acolorpos +
        " " +
        a_colors_str[v_acolorpos] +
        " " +
        a_colors[v_acolorpos].toString(16);
