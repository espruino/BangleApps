  ...
  settingsMenu(
    function (s, reset) {
      ...
      if (reset) {
        ...
        getSecondsTime() - settingsMenuOpened < 500 ||
        correctionMode
      ) {
...
function tiebreakMaxScore() {
  return Math.max(
    settings.maxScoreTiebreakMaxScore,
    settings.maxScoreTiebreakWinScore
  );
}
...
function shouldTiebreak() {
  return (
    settings.enableMaxScoreTiebreak &&
...
function tiebreakWon(set, player) {
  ...
  let reachedMaxScore =
    settings.maxScoreTiebreakEnableMaxScore && pScore >= tiebreakMaxScore();
...
function score(player) {
  ...
  if (correctionMode) {
    ...
    if (shouldTiebreak()) {
      ...
      if (tScores[player] === 4 && tScores[~~!player] === 5) {
        // DC : AD
        tScores[~~!player]--;
      } else if (tScores[player] === 2 && tScores[~~!player] === 3) {
        // 30 : 40
        tScores[0] = 4;
        tScores[1] = 4;
      } else if (tScores[player] === 3 || tScores[player] === 5) {
        // 40 / AD
        tScores[0] = 0;
        tScores[1] = 0;
        scores[cSet][player]++;
...
function draw() {
  ...
  let setsOnCurrentPage = Math.min(sets(), setsPerPage());
  for (let set = firstShownSet; set < lastShownSet; set++) {
    if (set < 0) continue;

    let y =
      ((h - 15) / (setsOnCurrentPage + 1)) * (set - firstShownSet + 1) + 5;
    ...
    for (let p = 0; p < 2; p++) {
      if (!setWon(set, p === 0 ? 1 : 0) || matchEnded()) {
        ...
        if (settings.enableTennisScoring && set === cSet && !shouldTiebreak()) {
          ...
          g.drawString(formatNumber(tennisScores[tScores[p]]), bigNumX, y);
        } else if (shouldTiebreak() && set === cSet) {
          g.setFontAlign(0, 0);
          g.setFont("7x11Numeric7Seg", 3);
          g.drawString(formatNumber(scores[set][3 + p], 3), bigNumX, y);
        } else {
          g.setFontAlign(0, 0);
          g.setFont("7x11Numeric7Seg", 3);
          g.drawString(formatNumber(scores[set][p]), bigNumX, y);
        }

        if (
          (shouldTiebreak() || settings.enableTennisScoring) &&
          set === cSet
        ) {
          g.setFontAlign(p === 0 ? 1 : -1, 0);
          g.setFont("7x11Numeric7Seg", 1);
          g.drawString(formatNumber(scores[set][p]), smallNumX, y);
        } else if (
          (scores[set][3] !== 0 || scores[set][4] !== 0) &&
          set !== cSet
        ) {
          g.setFontAlign(p === 0 ? 1 : -1, 0);
          g.setFont("7x11Numeric7Seg", 1);
          g.drawString(formatNumber(scores[set][3 + p], 3), smallNumX, y);
        }
      }
    }
  }
