...
function has_commands() {
  return (
    command_stack_high_priority.length > 0 ||
...
if (g.getHeight() > 200) setRowDisplays(50, [40, 30, 30, 30, 40]);
else setRowDisplays(34, [35, 25, 25, 25, 35]);
...
function changeFormatter() {
  ...
  reset_clock(true);
  draw_clock();
  command_stack_high_priority.unshift(function () {
    ...
    format_name_display.moveToX(
      -date_formatter.name().length * format_name_display.font_size
    );
  });
}
...
function display_row(display, txt) {
  ...
  if (display.txt == null || display.txt == "") {
    ...
    command_stack_high_priority.push(function () {
      //console.log("move out:" + txt);
      display.onFinished(next_command);
      display.moveToX(-display.txt.length * display.font_size);
    });
    command_stack_low_priority.push(function () {
...
function load_settings() {
  ...
  if (!setScheme) setColorScheme(color_schemes[color_scheme_index]);
}
