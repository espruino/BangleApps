...
- `Show Menu Hidden` (default false) Whether to show the menu options for `Hidden` when editing/creating an alarm and whether to show `hidden` alarms in the main menu. They will show in group menus either way. If `Show Menu Hidden` and `Show Menu Group` are both true, un-grouped hidden alarms will be put in a "Hidden" group menu. See `Internals -> hidden` in the [shed library README](https://github.com/espruino/BangleApps/blob/master/apps/sched/README.md) for more info.
...
- `Show Hidden Alarms` (default false) Whether to show `hidden` alarms in the main menu. They will show in group menus either way. If `Show Hidden Alarms` and `Show Menu Group` are both true, ungrouped hidden alarms will be put in a "Hidden" group menu. See `Internals -> hidden` in the [shed library README](https://github.com/espruino/BangleApps/blob/master/apps/sched/README.md) for more info.
