...
- `Show Menu Hidden` (default false) Whether to show `hidden` alarms in the main menu, as well as whether to show the menu options for `Hidden` when editing/creating an alarm. If `Show Menu Hidden` and `Show Menu Group` are both set then ungrouped hidden alarms will be found in the "Hidden" sub-menu. See `Internals -> hidden` in the [shed library README](https://github.com/espruino/BangleApps/blob/master/apps/sched/README.md) for more info.
