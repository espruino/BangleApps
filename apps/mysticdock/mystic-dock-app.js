...
const settings =
  require("Storage").readJSON("mysticdock.json", 1) ||
...
function drawImage() {
  setColor();
  g.drawPoly(
    [xyCenter - 100, ypos[2], xyCenter + 100, ypos[2], xyCenter, ypos[2] + 30],
    true
  );
}
...
function drawClock() {
  ...
  if (lastButtonPressTime && (d.getTime() - lastButtonPressTime) / 1000 < 5) {
    ...
    g.drawString(
      `${hours}${d.getSeconds() % 2 ? " " : ":"}${minutes}`,
      xyCenter - (settings.use12Hour ? 15 : 0),
      y,
      true
    );
    g.setFont(font, dataFontSize);
    ...
    g.drawString(
      `BATT: ${
        E.getBattery() === 100 ? "100" : ("0" + E.getBattery()).substr(-2)
      }%`,
      xyCenter,
      y + 35,
      true
