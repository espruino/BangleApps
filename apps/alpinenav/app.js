...
function draw() {
  if (pause_tracker) g.setColor(background_colour);
  else g.setColor(foregound_colour);
  g.setFont("6x8", 2);
  g.setFontAlign(0, 1);
...
function cull_array() {
  for (var i = 2; i <= waypoints.length; i += 2) waypoints.splice(i, 1);
}
...
function process_and_display() {
  ...
  if (waypoints.length > max_array_size) cull_array();
  rescale();
  if (display_waypoints.length > 0) {
    for (let x = 0; x < display_waypoints.length - 1; x++) {
      ...
      g.setColor(foregound_colour);
      g.drawLine(
        display_waypoints[x].lon,
        display_waypoints[x].lat,
        display_waypoints[x + 1].lon,
        display_waypoints[x + 1].lat
      );
    }
    g.reset();
    g.setColor(position_colour);
    g.fillCircle(
      display_waypoints[display_waypoints.length - 1].lon,
      display_waypoints[display_waypoints.length - 1].lat,
      3
    );
  }
}
...
Bangle.on("GPS", function (g) {
  if (g.fix) {
    if (waypoints.length == 0) {
      ...
      var csv = [d, origin_lat, origin_lon, current_speed, current_alt];
      file.write(csv.join(",") + "\n");
      var temp = Object.create(waypoint);
      temp.lat = 0;
      temp.lon = 0;
...
Bangle.on("mag", function (m) {
  ...
  g.setColor(foregound_colour);
  g.setFont("6x8", 2);
  if (compass_heading < 100) compass_heading = " " + compass_heading.toString();
