...
exports.pushMessage = function (event) {
  var messages,
    inApp = "undefined" != typeof MESSAGES;
  if (inApp) messages = MESSAGES;
  // we're in an app that has already loaded messages
  // no app - load messages
  else messages = require("Storage").readJSON("messages.json", 1) || [];
  ...
  if (event.t == "remove") {
    ...
    if (event.t == "add") {
      if (event.new === undefined) {
        // If 'new' has not been set yet, set it
