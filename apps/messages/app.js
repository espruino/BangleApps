...
function getNotificationImage() {
  return atob("FBSBAH//7//////////////////////////////////////////////3//4PgADgAA4AAAAA");
}
function getFBIcon() {
...
function checkMessages(options) {
  ...
  if (!MESSAGES.length) {
    if (!options.clockIfNoMsg) return E.showPrompt("No Messages",{
      title:"Messages",
      img:require("heatshrink").decompress(atob("")),
      buttons : {"Ok":1}
    }).then(() => { load() });
    return load();
  }
